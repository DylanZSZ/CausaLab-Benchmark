...
print_runtime_header "run_react_simple-mem_parallel"
...
echo "Total batches: $(( (TOTAL_GRAPHS + BATCH_SIZE - 1) / BATCH_SIZE ))"
...
for ((batch_start = 0; batch_start < TOTAL_GRAPHS; batch_start += BATCH_SIZE)); do
  ...
  for ((idx = batch_start; idx < batch_end; idx++)); do
    ...
    for ENV_SEED in "${SEEDS[@]}"; do
      ...
      if [[ "$DRY_RUN" == "1" ]]; then
        echo "[DRY_RUN] graph=$CURRENT_GRAPH_ID seed=$ENV_SEED output=$SUBDIR threadid=$UNIQUE_THREADID_OFFSET"
        continue
      fi
  ...
  if [[ "$DRY_RUN" != "1" ]]; then
    for pid in "${BATCH_PIDS[@]}"; do
      if ! wait "$pid"; then
        ...
        seed="${PID_TO_SEED[$pid]}"
        echo "  FAILED: graph $graph_id, seed $seed (PID: $pid)" >&2
  ...
  if [[ $batch_failed -eq 0 ]]; then
    ...
    echo "  Batch $batch_num completed with failures"
  fi
...
echo "============================================================================"
echo "Total graphs processed: $TOTAL_GRAPHS"
echo "Seeds per graph: $NUM_SEEDS"
echo "Total tasks: $((TOTAL_GRAPHS * NUM_SEEDS))"
...
echo "Some tasks failed: ${FAILED_GRAPHS[*]}" >&2
exit 1
