...
export HIDDEN_BASE="${HIDDEN_BASE:-output_dir/react_simple-mem/obs_CausalHiddenFreqNode_${SAFE_MODEL}/4nodes_hidden_freq_node_priority}"

print_runtime_header "run_hidden_frequency_node_priority_full"
echo "Model: ${MODEL}"
...
HIDDEN_SPECS=(
  "4nodes_hidden_freqnode_n1.jsonl:hidden_freqnode_n1"
  "4nodes_hidden_freqnode_n2.jsonl:hidden_freqnode_n2"
  "4nodes_hidden_freqnode_n3.jsonl:hidden_freqnode_n3"
)

for spec in "${HIDDEN_SPECS[@]}"; do
  IFS=":" read -r graph_group output_tag <<< "$spec"
  echo "============================================================================"
  echo "Running hidden frequency-node-priority variant: ${output_tag}"
  ...
  env \
    ...
    REACTOR_TASK_PROMPT_MODE="dsl_hidden_freqnode" \
    PROMPT_NAME="dsl_hidden_freqnode" \
