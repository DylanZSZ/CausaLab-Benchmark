...
export HIDDEN_BASE="${HIDDEN_BASE:-output_dir/react_simple-mem/obs_CausalHiddenFreqPrio_${SAFE_MODEL}/4nodes_hidden_freq_priority}"

print_runtime_header "run_hidden_frequency_priority_full"
echo "Model: ${MODEL}"
...
HIDDEN_SPECS=(
  "4nodes_hidden_freqprio_pm5.jsonl:hidden_freqprio_pm5"
  "4nodes_hidden_freqprio_n1.jsonl:hidden_freqprio_n1"
  "4nodes_hidden_freqprio_n2.jsonl:hidden_freqprio_n2"
  "4nodes_hidden_freqprio_n3.jsonl:hidden_freqprio_n3"
)

for spec in "${HIDDEN_SPECS[@]}"; do
  IFS=":" read -r graph_group output_tag <<< "$spec"
  echo "============================================================================"
  echo "Running hidden frequency-priority variant: ${output_tag}"
  ...
  env \
    ...
    REACTOR_TASK_PROMPT_MODE="dsl_hidden_freq" \
    PROMPT_NAME="dsl_hidden_freq" \
