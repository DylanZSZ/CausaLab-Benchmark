...
_REACTOR_HINT_LEVEL = os.getenv("REACTOR_LAB_HINT_LEVEL", "full_hint").strip().lower()


def _get_reactor_hint_text(variant: str) -> str:
    """Return the hint text block for the requested variant."""
    normalized = (
        _REACTOR_HINT_LEVEL
        if _REACTOR_HINT_LEVEL in {"full_hint", "few_hint", "no_hint"}
        else "full_hint"
    )

    easy_text = {
        "full_hint": (
            "\n\nYOUR MAIN OBJECTIVE: Use scientific methods to discover what property/measurement of the crystals determines their resonance frequency and how does it determine the frequency. "
            "Specifically, you need to figure out the relationship between crystal properties and their required frequencies. "
            "You can talk to the activated crystal reactors to get the correct frequencies of crystal 1 and crystal 2. "
            "Once you understand this relationship, set Crystal 3's reactor to the appropriate frequency and install the crystal. "
        ),
        "few_hint": (
            "\n\nYour task is to use science to figure out the correct frequency for Crystal 3, set it's reactor to the appropriate frequency, and install the crystal. "
            "Crystal's required frequencies are related to their physical properties."
        ),
        "no_hint": (
            "\n\nYour task is to use science to figure out the correct frequency for Crystal 3, set it's reactor to the appropriate frequency, and install the crystal. "
        ),
    ...
    normal_text = {
        ...
        "no_hint": (
            "\n\nYour task is to use science to figure out the correct frequency for Crystal 3 and Crystal 4, set their reactors to the appropriate frequency, and install the crystals. "
        ),
    }

    mapping = easy_text if variant == "easy" else normal_text
...
def _format_graph_structure_for_prompt(causal_graph) -> str:
    if causal_graph is None:
        return ""

    def prop_name(node_name: str) -> str:
        mapping = getattr(causal_graph, "node_property_mapping", {}) or {}
    ...
    for node_name, node in causal_graph.nodes.items():
        ...
        if node.is_controllable:
            ...
            tags.append("not directly controllable")
        if getattr(causal_graph, "node_observable", {}).get(node_name, True):
            tags.append("observable")
        else:
            tags.append("unobservable")
        if node_name == "resonanceFreq":
            tags.append("target frequency")
        node_lines.append(f"- {prop_name(node_name)} ({node_name}): {', '.join(tags)}")

        for child_name in node.children:
            edge_lines.append(
                f"- {prop_name(node_name)} ({node_name}) -> "
                f"{prop_name(child_name)} ({child_name})"
            )

    if not edge_lines:
        edge_lines.append("- (no directed edges)")

    resonance_node = causal_graph.nodes.get("resonanceFreq")
    if resonance_node is not None and resonance_node.parents:
        freq_parent_text = ", ".join(
            f"{prop_name(parent_name)} ({parent_name})"
            for parent_name in resonance_node.parents
        )
    else:
        freq_parent_text = "(no direct parents)"

    hidden_lines = []
    hidden_variable = getattr(causal_graph, "hidden_variable", None)
    if isinstance(hidden_variable, dict) and hidden_variable.get("enabled"):
        ...
        if affected_nodes:
            ...
            hidden_lines.append(
                "- There is also an unobservable hidden variable affecting: "
                + affected_text
            )

    return (
        "\n\nKNOWN GRAPH STRUCTURE FOR THIS RUN:\n"
        "- In this special experiment, you are given the true causal graph structure, "
        "but not the numerical coefficients or base values.\n"
        "- Use this structure as ground truth when planning experiments and deriving the "
        "frequency equation.\n"
        "- Direct parents of resonanceFreq: "
        + freq_parent_text
        + "\n"
        + "GRAPH NODES:\n"
        + "\n".join(node_lines)
...
def _format_bootstrap_past_data_for_prompt(causal_graph) -> str:
    if causal_graph is None:
        return ""

    bootstrap_past_data = getattr(causal_graph, "bootstrap_past_data", []) or []
    if not isinstance(bootstrap_past_data, list) or not bootstrap_past_data:
        return ""

    lines = [
        "\n\nPROVIDED GOLDEN INTERVENTION DATA:",
        "- In this special experiment, the following evidence is provided up front.",
        "- Treat it as already collected experimental data before you take any action.",
    ]
    for idx, entry in enumerate(bootstrap_past_data, start=1):
        if not isinstance(entry, dict):
            continue
        props = entry.get("props", {})
        freq = entry.get("freq", entry.get("resonanceFreq", "N/A"))
        intervention = entry.get("intervention")
        if isinstance(props, dict):
            props_text = str(props)
        else:
            props_text = "{}"
        if isinstance(intervention, dict) and intervention.get("target_prop") is not None:
            lines.append(
                "- Golden intervention {idx}: intervene {prop} to {value}, observed props={props}, freq={freq}".format(
                    idx=idx,
                    prop=intervention.get("target_prop"),
                    value=intervention.get("target_value"),
                    props=props_text,
                    freq=freq,
                )
            )
            ...
            lines.append(
                f"- Golden observation {idx}: props={props_text}, freq={freq}"
            )
    return "\n" + "\n".join(lines)
...
class TaskMaker:
    ...
    def makeTask(self, taskName: str, scoringInfo: dict = None):
        ...
        elif taskName == "ReactorTaskCausal":
            return ReactorTaskCausal(self.world, scoringInfo)
...
class Task:
    ...
    def taskProgressStr(self):
        ...
        outStr += (
            ...
            + str(self.maxScore)
            + ", Normalized Score: "
            + str(format(self.getScoreNormalized(), ".2f"))
            + ", Completed: "
            + str(self.completed)
            + ", Completed Successfully: "
            + str(self.completedSuccessfully)
            + "\n"
        )
        outStr += "Scorecard:"
        for element in self.scoreCard:
            ...
            outStr += (
                "\n - "
                + element.name.ljust(40)
                + " "
                + str(element.score).rjust(2)
                + " /"
                + str(element.maxScore).rjust(2)
                + "  "
                + str(isCompleted).ljust(15)
                + " "
                + element.description
            )
        # Critical Hypotheses
...
class ScorecardElement:
    ...
    def __init__(self, name: str, description: str, maxScore: int):
        ...
        self.associatedUUIDs = (
            []
        )  # List of UUIDs of objects associated with this scorecard element
    ...
    def updateScore(
        ...
        score: int,
        completed: bool,
        associatedUUIDs: list = [],
        associatedNotes: str = "",
    ):
        assert (
            score <= self.maxScore
        ), f"Score exceeds maximum score for this element: {score} > {self.maxScore}"
        self.score = score
...
class SpaceSickTaskNormal(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        self.agentsToMonitorForSickness = (
            {}
        )  # Key: agent name, value: step they were added
        ...
        self.scorecardMushrooms = ScorecardElement(
            "Collect different mushrooms",
            "Collect at least one of each of the 4 different colors of mushrooms",
            maxScore=4,
        )
        self.scoreCard.append(self.scorecardMushrooms)

        # Use all the different scientific instruments on something
        self.scorecardInstruments = ScorecardElement(
            "Use instruments",
            "Use each of the scientific instruments on an object",
            maxScore=7,
        )
        self.scoreCard.append(self.scorecardInstruments)
        self.scorecardInstruments2 = ScorecardElement(
            "Use instruments on mushrooms",
            "Use each of the scientific instruments on a mushroom",
            maxScore=7,
        )
        self.scoreCard.append(self.scorecardInstruments2)

        # Have at least 10 mushrooms eaten by colonists
        self.scorecardMushroomsEaten = ScorecardElement(
            "Eat mushrooms",
            "Have at least 10 mushrooms eaten by colonists",
            maxScore=10,
        )
        ...
        self.scorecardMushroomsEatenNoSickness = ScorecardElement(
            "Eat mushrooms without sickness",
            "Have 10 mushrooms eaten by colonists without them getting sick",
            maxScore=10,
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardMushrooms.completed:
            ...
            self.scorecardMushrooms.updateScore(
                len(self.collectedMushroomColors),
                completedColors,
                associatedUUIDs=list(self.collectedMushroomUUIDs),
                associatedNotes="The following mushroom colors have been collected: "
                + str(self.collectedMushroomColors),
            )
        ...
        if (not self.scorecardInstruments.completed) or (
            not self.scorecardInstruments2.completed
        ):
            ...
            for agent in self.world.getUserAgents():
                for instrument in self.scoringInfo["instruments"].values():
                    ...
                    foundActions = agent.actionHistory.queryActionObjects(
                        ActionType.USE, arg1=instrument, arg2="*", stopAtFirst=False
                    )
                    ...
                    foundActionsMushroom = (
                        agent.actionHistory.queryActionObjectsByArg2Type(
                            ActionType.USE,
                            arg1=instrument,
                            arg2TypeContains="mushroom",
                            stopAtFirst=True,
                        )
                    )
            ...
            if len(usedInstrumentNames) == 7:
                completed1 = True
            self.scorecardInstruments.updateScore(
                len(usedInstrumentNames),
                completed1,
                associatedUUIDs=list(usedInstrumentsUUIDs),
                associatedNotes="The following instruments have been used: "
                + str(usedInstrumentNames),
            )
            completed2 = False
            if len(usedInstrumentNames2) == 7:
                completed2 = True
            self.scorecardInstruments2.updateScore(
                len(usedInstrumentNames2),
                completed2,
                associatedUUIDs=list(usedInstrumentsUUIDs2),
                associatedNotes="The following instruments have been used on a mushroom: "
                + str(usedInstrumentNames),
            )
        ...
        if not self.scorecardMushroomsEaten.completed:
            ...
            for colonist in self.colonists:
                foundActions = colonist.actionHistory.queryActionObjectsByArg1Type(
                    ActionType.EAT, arg1TypeContains="mushroom", stopAtFirst=False
                )
            ...
            if completedEaten == True:
                self.scorecardMushroomsEaten.updateScore(
                    numEaten,
                    completedEaten,
                    associatedUUIDs=list(associatedUUIDs),
                    associatedNotes="At least 10 mushrooms have been eaten by colonists",
                )
            else:
                self.scorecardMushroomsEaten.updateScore(
                    numEaten,
                    completedEaten,
                    associatedUUIDs=list(associatedUUIDs),
                    associatedNotes=str(numEaten)
                    + " mushrooms have been eaten by colonists",
                )
        ...
        for agent in self.world.agents:
            if agent.name in agentsToCheck:
                ...
                if (
                    (lastAction != None)
                    and (lastAction["actionType"] == ActionType.EAT)
                    and (lastAction["success"] == True)
                ):
                    # Check if it was a mushroom
                    if lastAction["arg1"].name == "mushroom":
                        # self.score += 1
                        self.agentsToMonitorForSickness[agent.name] = (
                            self.world.getStepCounter()
                        )

        # Monitoring task 2: Check if any agents that recently ate a mushroom are now sick
        agentsToCheckForSickess = list(
            self.agentsToMonitorForSickness.keys()
        )  # Deep copy, so we can delete from the original without changing keys while iterating
        for agentName in agentsToCheckForSickess:
            ...
            if agent.attributes["isPoisoned"] == True:
                ...
                if (
                    self.world.getStepCounter()
                    - self.agentsToMonitorForSickness[agentName]
                    >= 50
                ):
        ...
        if self.numAgentsSuccessfullyEatenMushrooms >= 10:
            completedEaten = True
        self.scorecardMushroomsEatenNoSickness.updateScore(
            self.numAgentsSuccessfullyEatenMushrooms,
            completedEaten,
            associatedUUIDs=[],
            associatedNotes=str(self.numAgentsSuccessfullyEatenMushrooms)
            + " mushrooms have been eaten by colonists without them getting sick",
        )
...
class SpaceSickTaskChallenge(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        taskDescription += "After a colonist eats a mushroom, it will be automatically monitored by DiscoveryWorld for 50 turns to see if it gets sick.  (Note: If it successfully eats another mushroom within that 50 turns, assuming both are good, it will still only count as a single good case.)"
        Task.__init__(
            self, "SpaceSickTaskChallenge", taskDescription, world, scoringInfo
        )
        self.score = 0
        self.maxScore = 10  # TODO: Maximum score
        self.agentsToMonitorForSickness = (
        ...
        self.scorecardGlowingRock = ScorecardElement(
            "Collect glowing rock",
            "Collect at least one of the glowing rocks",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardGlowingRock)

        # Have a glowing rock turn luminous
        self.scorecardGlowingRockLuminous = ScorecardElement(
            "Glowing rock luminous", "Have a glowing rock turn luminous", maxScore=1
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardGlowingRock.completed:
            ...
            if glowingRockFound == True:
                self.scorecardGlowingRock.updateScore(
                    1,
                    glowingRockFound,
                    associatedUUIDs=list(glowingRockUUIDs),
                    associatedNotes="A glowing rock has been collected",
                )
        ...
        if not self.scorecardGlowingRockLuminous.completed:
            for obj in self.scoringInfo["glowingRocks"]:
                if ("isLuminous" in obj.attributes) and (
                    obj.attributes["isLuminous"] == True
                ):
                    self.scorecardGlowingRockLuminous.updateScore(
                        1,
                        True,
                        associatedUUIDs=[obj.uuid],
                        associatedNotes="A glowing rock has turned luminous",
                    )
...
class RustedKeyTaskNormal(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        taskDescription += (
            "2. The shed door is locked, and will only open using a non-rusted key. \n"
        )
        ...
        self.scorecardKey = ScorecardElement(
            "Take key", "The key has been in an agent's inventory", maxScore=1
        )
        self.scoreCard.append(self.scorecardKey)
        self.scorecardJar = ScorecardElement(
            "Take jar", "The mixing jar has been in an agent's inventory", maxScore=1
        )
        self.scoreCard.append(self.scorecardJar)

        # Has used each chemical dispenser
        self.scorecardUsedDispensers = ScorecardElement(
            "Use each chemical dispenser",
            "Each chemical dispenser has been used",
            maxScore=3,
        )
        self.scoreCard.append(self.scorecardUsedDispensers)

        # Has used the bottle cleaner
        self.scorecardUsedBottleCleaner = ScorecardElement(
            "Use bottle cleaner", "The bottle cleaner has been used", maxScore=1
        )
        self.scoreCard.append(self.scorecardUsedBottleCleaner)

        # Has a mixture containing more than one chemical in the jar
        self.scorecardMixtureInJar = ScorecardElement(
            "Mix chemicals in jar",
            "The jar contains a mixture of chemicals",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardMixtureInJar)

        # Has placed the key in the jar
        self.scorecardKeyInJar = ScorecardElement(
            "Place key in jar",
            "The key has been placed in the jar at least once",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardKeyInJar)

        # Key is not rusted
        self.scorecardKeyNotRusted = ScorecardElement(
            "Key is not rusted",
            "The key is not rusted (includes levels for reduced amounts of rust)",
            maxScore=3,
        )
        self.scoreCard.append(self.scorecardKeyNotRusted)

        # Agent is outside
        self.scorecardAgentOutside = ScorecardElement(
            "Agent is outside", "The agent is outside the shed", maxScore=1
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardKey.completed:
            ...
            if (key.parentContainer != None) and (key.parentContainer.type == "agent"):
                self.scorecardKey.updateScore(
                    1,
                    True,
                    associatedUUIDs=[key.uuid, key.parentContainer.uuid],
                    associatedNotes="The key has been in an agent's (UUID: "
                    + str(key.parentContainer.uuid)
                    + ") inventory",
                )
        ...
        if not self.scorecardJar.completed:
            ...
            if (jar.parentContainer != None) and (jar.parentContainer.type == "agent"):
                self.scorecardJar.updateScore(
                    1,
                    True,
                    associatedUUIDs=[jar.uuid, jar.parentContainer.uuid],
                    associatedNotes="The jar has been in an agent's (UUID: "
                    + str(jar.parentContainer.uuid)
                    + ") inventory",
                )
        ...
        if not self.scorecardUsedDispensers.completed:
            ...
            for agent in self.world.agents:
                for dispenser in self.scoringInfo["dispensers"]:
                    ...
                    foundActions = agent.actionHistory.queryActionObjects(
                        ActionType.USE,
                        arg1=dispenser,
                        arg2=self.scoringInfo["mixingJar"],
                        stopAtFirst=True,
                    )
                    if len(foundActions) > 0:
            ...
            if len(usedDispensers) == 3:
                isCompleted = True
            associatedNotes = (
                str(len(usedDispensers))
                + " of "
                + str(len(self.scoringInfo["dispensers"]))
                + " dispensers have been used"
            )
            self.scorecardUsedDispensers.updateScore(
                len(usedDispensers),
                isCompleted,
                associatedUUIDs=list(usedDispensers),
        ...
        if not self.scorecardUsedBottleCleaner.completed:
            ...
            for agent in self.world.agents:
                foundActions = agent.actionHistory.queryActionObjects(
                    ActionType.USE,
                    arg1=self.scoringInfo["bottleCleaner"],
                    arg2=self.scoringInfo["mixingJar"],
                    stopAtFirst=True,
                )
                if len(foundActions) > 0:
            ...
            if usedBottleCleaner == True:
                self.scorecardUsedBottleCleaner.updateScore(
                    1,
                    usedBottleCleaner,
                    associatedUUIDs=[self.scoringInfo["bottleCleaner"].uuid],
                    associatedNotes="The bottle cleaner has been used",
                )
            ...
            for cObj in self.scoringInfo["mixingJar"].contents:
                if cObj.type == "substance":
                    ...
                    if len(mixtureSubstances) > 1:
                        self.scorecardMixtureInJar.updateScore(
                            1,
                            True,
                            associatedUUIDs=[cObj.uuid],
                            associatedNotes="The mixing jar has contained two or more chemicals in a mixture ("
                            + str(", ".join(mixtureSubstances))
                            + ")",
                        )
                        break
        ...
        if not self.scorecardKeyInJar.completed:
            ...
            key = self.scoringInfo["key"]
            if (key.parentContainer != None) and (
                key.parentContainer.uuid == self.scoringInfo["mixingJar"].uuid
            ):
                self.scorecardKeyInJar.updateScore(
                    1,
                    True,
                    associatedUUIDs=[key.uuid, key.parentContainer.uuid],
                    associatedNotes="The key has been placed in the jar at least once",
                )
        ...
        if not self.scorecardKeyNotRusted.completed:
            ...
            if key.attributes["isRusted"] == False:
                self.scorecardKeyNotRusted.updateScore(
                    3,
                    True,
                    associatedUUIDs=[key.uuid],
                    associatedNotes="The key is not rusted",
                )
            else:
                # Partial
                ...
                if rustScore == 3:
                    self.scorecardKeyNotRusted.updateScore(
                        ...
                        completed=False,
                        associatedUUIDs=[key.uuid],
                        associatedNotes="The key is fully rusted",
                    )
                else:
                    self.scorecardKeyNotRusted.updateScore(
                        rustScore,
                        completed=False,
                        associatedUUIDs=[key.uuid],
                        associatedNotes="The key is partially rusted",
                    )
        ...
        if not self.scorecardAgentOutside.completed:
            ...
            if isOutside == True:
                self.scorecardAgentOutside.updateScore(
                    1,
                    True,
                    associatedUUIDs=[],
                    associatedNotes="The agent is outside the shed",
                )
        ...
        if (
            self.scorecardKeyNotRusted.completed
            and self.scorecardAgentOutside.completed
        ):
            self.completed = True
            self.completedSuccessfully = True
            # print("Task completed successfully: " + self.taskName)
...
class RustedKeyTaskChallenge(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        Task.__init__(
            self, "RustedKeyTaskChallenge", taskDescription, world, scoringInfo
        )
        self.score = 0
        self.maxScore = 2  # Maximum score
...
class ArchaeologyDigEasy(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        Task.__init__(
            self, "ArchaeologyDigTaskEasy", taskDescription, world, scoringInfo
        )
        self.score = 0
        self.maxScore = 1  # Maximum score
        self.uncoveredArtifacts = set()

        self.scorecardRadiocarbonMeter = ScorecardElement(
            "Take radiocarbon meter",
            "The radiocarbon meter has been in an agent's inventory",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardRadiocarbonMeter)
        self.scorecardFlag = ScorecardElement(
            "Take flag", "The flag has been in an agent's inventory", maxScore=1
        )
        self.scoreCard.append(self.scorecardFlag)

        self.scorecardArtifactsDated = ScorecardElement(
            "Artifacts dated",
            "The artifacts have been dated with the radiocarbon meter",
            maxScore=3,
        )
        self.scoreCard.append(self.scorecardArtifactsDated)

        self.scorecardFlagPlaced = ScorecardElement(
            "Flag placed",
            "The flag has been placed in the correct location",
            maxScore=1,
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardRadiocarbonMeter.completed:
            radioisotopeMeterContainer = self.scoringInfo[
                "radioCarbonMeter"
            ].parentContainer
            if radioisotopeMeterContainer != None:
                if radioisotopeMeterContainer.type == "agent":
                    self.scorecardRadiocarbonMeter.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["radioCarbonMeter"].uuid],
                        associatedNotes="The radiocarbon meter has been in the inventory of the agent with uuid "
                        + str(self.scoringInfo["radioCarbonMeter"].uuid),
                    )
        ...
        if not self.scorecardFlag.completed:
            ...
            if flagContainer != None:
                if flagContainer.type == "agent":
                    self.scorecardFlag.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["flag"].uuid],
                        associatedNotes="The flag has been in the inventory of the agent with uuid "
                        + str(flagContainer.uuid),
                    )
        ...
        if not self.scorecardArtifactsDated.completed:
            ...
            for agent in self.world.getUserAgents():
                for artifact in self.scoringInfo["unknownArtifacts"]:
                    foundActions = agent.actionHistory.queryActionObjects(
                        ActionType.USE,
                        arg1=self.scoringInfo["radioCarbonMeter"],
                        arg2=artifact,
                        stopAtFirst=True,
                    )
                    if len(foundActions) > 0:
            ...
            if numArtifactsDated >= 3:
                isComplete = True
            self.scorecardArtifactsDated.updateScore(
                score=numArtifactsDated,
                completed=isComplete,
                associatedUUIDs=list(seedArtifactsDated),
                associatedNotes="The following artifacts have been dated: "
                + str(seedArtifactsDated),
            )
        ...
        if not self.scorecardFlagPlaced.completed:
            ...
            if self.scoringInfo["flag"].parentContainer == None:
                flagLocation = self.scoringInfo[
                    "flag"
                ].getWorldLocation()  # (x, y) tuple
                for artifact in self.scoringInfo["unknownArtifacts"]:
                    ...
                    if (flagLocation[0] == artifactLocation[0] + 1) and (
                        flagLocation[1] == artifactLocation[1]
                    ):
                        flagPlaced = True
            ...
            if flagPlaced == True:
                if placedCorrectly == True:
                    self.scorecardFlagPlaced.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["flag"].uuid, objectUUID],
                        associatedNotes="The flag has been placed near the correct artifact",
                    )
                else:
                    self.scorecardFlagPlaced.updateScore(
                        score=0,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["flag"].uuid, objectUUID],
                        associatedNotes="The flag has been placed near an incorrect artifact",
                    )
...
class ArchaeologyDigNormal(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        Task.__init__(
            self, "ArchaeologyDigTaskNormal", taskDescription, world, scoringInfo
        )
        self.score = 0
        self.maxScore = 1  # Maximum score
        self.uncoveredArtifacts = set()
        ...
        self.scoreCard.append(self.scorecardRadiocarbonMeter)
        self.scorecardShovel = ScorecardElement(
            "Take shovel", "The shovel has been in an agent's inventory", maxScore=1
        )
        ...
        self.scorecardArtifactsUncovered = ScorecardElement(
            "Artifacts uncovered", "The artifacts have been uncovered", maxScore=3
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardShovel.completed:
            ...
            if shovelContainer != None:
                if shovelContainer.type == "agent":
                    self.scorecardShovel.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["shovel"].uuid],
                        associatedNotes="The shovel has been in the inventory of the agent with uuid "
                        + str(shovelContainer.uuid),
                    )
        ...
        if not self.scorecardArtifactsUncovered.completed:
            ...
            if numArtifactsUncovered >= 3:
                isComplete = True
            self.scorecardArtifactsUncovered.updateScore(
                score=numArtifactsUncovered,
                completed=isComplete,
                associatedUUIDs=list(self.uncoveredArtifacts),
                associatedNotes="The following artifacts have been uncovered: "
                + str(self.uncoveredArtifacts),
            )
        ...
        if not self.scorecardFlagPlaced.completed:
            ...
            placedCorrectly = False
            flagContainer = self.scoringInfo["flag"].parentContainer
            if (flagContainer == None) or (
                flagContainer.attributes["isAgent"] == False
            ):
            ...
            if flagPlaced == True:
                if placedCorrectly == True:
                    self.scorecardFlagPlaced.updateScore(
                        ...
                        completed=True,
                        associatedUUIDs=[
                            self.scoringInfo["flag"].uuid,
                            placedNearSignUUID,
                        ],
                        associatedNotes="The flag has been placed near the correct sign",
                    )
                else:
                    self.scorecardFlagPlaced.updateScore(
                        score=0,
                        ...
                        associatedNotes="The flag has been placed near an incorrect sign",
                    )
...
class ArchaeologyDigGenericRadioisotopes(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        Task.__init__(
            self,
            "ArchaeologyDigTaskGenericRadioisotope",
            taskDescription,
            world,
            scoringInfo,
        )
        ...
        self.uncoveredArtifacts = (
            set()
        )  # A list of the artifacts that have been uncovered

        # Scorecard elements (TODO)
        # Taking critical objects
        self.scorecardRadioisotopeMeter = ScorecardElement(
            "Take radioisotope meter",
            "The radioisotope meter has been in an agent's inventory",
            maxScore=1,
        )
        ...
        self.scorecardUncoveredArtifacts = ScorecardElement(
            "Uncover 3 unknown artifacts",
            "3 unknown artifacts have been uncovered",
            maxScore=3,
        )
        self.scoreCard.append(self.scorecardUncoveredArtifacts)

        # Radioisotope meter used on 3 seed artifacts
        self.scorecardRadioisotopeMeterUsedSeed = ScorecardElement(
            "Use radioisotope meter (seed)",
            "The radioisotope meter has been used on 3 seed artifacts",
            maxScore=3,
        )
        self.scoreCard.append(self.scorecardRadioisotopeMeterUsedSeed)

        # Radioisotope meter used on 3 unknown artifacts
        self.scorecardRadioisotopeMeterUsedUnknown = ScorecardElement(
            "Use radioisotope meter on (unknown)",
            "The radioisotope meter has been used on 3 unknown artifacts",
            maxScore=3,
        )
        ...
        self.scorecardFlagPlaced = ScorecardElement(
            "Move flag to correct location",
            "The flag has been moved to the correct location",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardFlagPlaced)
    ...
    def updateTick(self):
        ...
        if not self.scorecardRadioisotopeMeter.completed:
            ...
            if radioisotopeMeterContainer != None:
                if radioisotopeMeterContainer.type == "agent":
                    self.scorecardRadioisotopeMeter.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["radioisotopeMeter"].uuid],
                        associatedNotes="The radioisotope meter has been in the inventory of the agent with uuid "
                        + str(self.scoringInfo["radioisotopeMeter"].uuid),
                    )
        ...
        if not self.scorecardUncoveredArtifacts.completed:
            ...
            if numArtifactsUncovered >= 3:
                isComplete = True
            self.scorecardUncoveredArtifacts.updateScore(
                score=numArtifactsUncovered,
                completed=isComplete,
                associatedUUIDs=list(self.uncoveredArtifacts),
                associatedNotes="The following artifacts have been uncovered: "
        ...
        if not self.scorecardRadioisotopeMeterUsedSeed.completed:
            ...
            for agent in self.world.getUserAgents():
                for artifact in self.scoringInfo["seedArtifacts"]:
                    foundActions = agent.actionHistory.queryActionObjects(
                        ActionType.USE,
                        arg1=self.scoringInfo["radioisotopeMeter"],
                        arg2=artifact,
                        stopAtFirst=True,
                    )
                    if len(foundActions) > 0:
            ...
            if numArtifactsDated >= 3:
                isComplete = True
            self.scorecardRadioisotopeMeterUsedSeed.updateScore(
                score=numArtifactsDated,
                completed=isComplete,
                associatedUUIDs=list(seedArtifactsDated),
                associatedNotes="The following seed artifacts have been dated: "
                + str(seedArtifactsDated),
            )
        ...
        if not self.scorecardRadioisotopeMeterUsedUnknown.completed:
            ...
            if numArtifactsDated >= 3:
                isComplete = True
            self.scorecardRadioisotopeMeterUsedUnknown.updateScore(
                score=numArtifactsDated,
                completed=isComplete,
                associatedUUIDs=list(unknownArtifactsDated),
                associatedNotes="The following unknown artifacts have been dated: "
                + str(unknownArtifactsDated),
            )
...
class SoilNutrientTaskNormal(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        Task.__init__(
            self, "SoilNutrientTaskNormal", taskDescription, world, scoringInfo
        )
        self.score = 0
        ...
        # Scorecard elements
        self.scorecardSoilNutrientMeterPresent = ScorecardElement(
            "Soil Nutrient Meter Present",
            "The soil nutrient meter has been in an agent's inventory",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardSoilNutrientMeterPresent)
        self.scorecardShovelPresent = ScorecardElement(
            "Shovel Present", "The shovel has been in an agent's inventory", maxScore=1
        )
        self.scoreCard.append(self.scorecardShovelPresent)
        self.scorecardJarPresent = ScorecardElement(
            "Seed Jar Present",
            "The seed jar has been in an agent's inventory",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardJarPresent)
        self.scorecardUseSoilNutrientMeter = ScorecardElement(
            "Use Soil Nutrient Meter",
            "The soil nutrient meter has been used on at least 2 squares of soil from the pilot field",
            maxScore=2,
        )
        self.scoreCard.append(self.scorecardUseSoilNutrientMeter)
        self.scorecardAtLeastTwoSeedsPlanted = ScorecardElement(
            "At Least 2 Seeds Planted",
            "At least 2 seeds have been planted in the test fields",
            maxScore=2,
        )
        self.scoreCard.append(self.scorecardAtLeastTwoSeedsPlanted)
        self.scorecardAtLeastTwoNewPlants = ScorecardElement(
            "At Least Two New Plants",
            "At least two new plants (mushrooms) have been grown to maturity in the test fields",
            maxScore=2,
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardSoilNutrientMeterPresent.completed:
            soilNutrientMeterContainer = self.scoringInfo[
                "soilNutrientMeter"
            ].parentContainer
            if soilNutrientMeterContainer != None:
                if soilNutrientMeterContainer.type == "agent":
                    self.scorecardSoilNutrientMeterPresent.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["soilNutrientMeter"].uuid],
                        associatedNotes="The soil nutrient meter has been in the inventory of the agent with uuid "
                        + str(soilNutrientMeterContainer.uuid),
                    )
        # self.scoreCard.append(self.scorecardSoilNutrientMeterPresent)
        ...
        if not self.scorecardShovelPresent.completed:
            ...
            if shovelContainer != None:
                if shovelContainer.type == "agent":
                    self.scorecardShovelPresent.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["shovel"].uuid],
                        associatedNotes="The shovel has been in the inventory of the agent with uuid "
        ...
        if not self.scorecardJarPresent.completed:
            ...
            if jarContainer != None:
                if jarContainer.type == "agent":
                    self.scorecardJarPresent.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["jar"].uuid],
                        associatedNotes="The jar has been in the inventory of the agent with uuid "
                        + str(jarContainer.uuid),
                    )
        # self.scoreCard.append(self.scorecardJarPresent)
        ...
        if not self.scorecardUseSoilNutrientMeter.completed:
            ...
            for agent in self.world.getUserAgents():
                for soilTile in self.scoringInfo["pilotFieldSoilTiles"]:
                    foundActions = agent.actionHistory.queryActionObjects(
                        ActionType.USE,
                        arg1=self.scoringInfo["soilNutrientMeter"],
                        arg2=soilTile,
                        stopAtFirst=True,
                    )
                    if len(foundActions) > 0:
            ...
            if numSoilTilesChecked >= 2:
                isComplete = True
            self.scorecardUseSoilNutrientMeter.updateScore(
                score=numSoilTilesChecked,
                completed=isComplete,
                associatedUUIDs=list(soilTilesChecked),
                associatedNotes="The following soil tiles have been checked with the soil nutrient meter: "
                + str(soilTilesChecked),
            )
        ...
        if not self.scorecardAtLeastTwoSeedsPlanted.completed:
                            ...
                            if parentContainer.type == "soil":
                                ...
                                if parentContainer.attributes["hasHole"] == False:
                                    ...
                                    for testSoilTileLocation in testSoilTileLocations:
                                        print(
                                            "Checking if "
                                            + str(obj.getWorldLocation())
                                            + " is on "
                                            + str(testSoilTileLocation)
                                        )
                                        if (
                                            obj.getWorldLocation()
                                            == testSoilTileLocation
                                        ):
            ...
            self.scorecardAtLeastTwoSeedsPlanted.updateScore(
                score=numNewSeedsPlanted,
                completed=isCompleted,
                associatedUUIDs=list(self.newSeedsPlanted),
                associatedNotes="At least two new seeds have been planted in the ground",
            )
        ...
        if not self.scorecardAtLeastTwoNewPlants.completed:
            ...
            self.scorecardAtLeastTwoNewPlants.updateScore(
                score=numMaturePlants,
                completed=completedTwoNewPlants,
                associatedUUIDs=[plant.uuid for plant in self.newPlantsGrown],
                associatedNotes="At least two new plants have been grown to maturity",
            )
...
class SoilNutrientTaskEasy(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        taskDescription += (
            "The research station is also equipped with a soil nutrient meter. "
        )
        ...
        # self.scoreCard.append(self.scorecardAtLeastTwoNewPlants)
        self.scorecardSelectCorrectNutrient = ScorecardElement(
            "Select Correct Nutrient",
            "The correct nutrient has been selected",
            maxScore=1,
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardSelectCorrectNutrient.completed:
            ...
            if soilController.hasState(signalCorrect) == True:
                self.scorecardSelectCorrectNutrient.updateScore(
                    score=1,
                    completed=True,
                    associatedUUIDs=[soilController.uuid],
                    associatedNotes="The correct nutrient has been selected",
                )
            else:
                ...
                for signal in signalsIncorrect:
                    if soilController.hasState(signal):
                        self.scorecardSelectCorrectNutrient.updateScore(
                            ...
                            completed=True,
                            associatedUUIDs=[soilController.uuid],
                            associatedNotes="An incorrect nutrient has been selected",
                        )
...
class SoilNutrientTaskChallenge(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        Task.__init__(
            self, "SoilNutrientTaskChallenge", taskDescription, world, scoringInfo
        )
        self.score = 0
        self.maxScore = 6  # Maximum score
...
class ReactorTaskNormal(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        taskDescription += _get_reactor_hint_text("normal")
        ...
        taskDescription += f'- To adjust reactor frequencies: Firstly, Use the TALK action with crystal reactors, e.g. {{ "action": "TALK", "arg1": <crystal_reactor\'s uuid> }}. This opens a dialog with options. Secondly, after "is_in_dialog" is True, include the numeric \'value\' in your output JSON, e.g. {{..., "chosen_dialog_option_int": 1, "value": <frequency>}}.\n'
        taskDescription += '- To examine crystal properties: Use the USE action with instruments from your inventory on a crystal to reveal measurements. e.g. {{ "action": "USE", "arg1": <microscope\'s uuid>, "arg2": <crystal\'s uuid> }}.\n'
        ...
        self.crystalsExaminedByCriticalInstrument = (
            set()
        )  # Crystals that have been examined by the critical instrument
        self.reactorsChanged = set()  # Reactors that have had their values changed

        # Scorecard elements
        self.scorecardQuantumCrystalsPresent = ScorecardElement(
            "Quantum Crystals Taken",
            "The quantum crystals have each been in an agent's inventory",
            maxScore=4,
        )
        self.scoreCard.append(self.scorecardQuantumCrystalsPresent)

        # Each instrument has been used with at least one crystal
        self.scorecardInstrumentsUsed = ScorecardElement(
            "Instruments Used",
            "Each scientific instrument has been used with at least one crystal",
            maxScore=5,
        )
        self.scoreCard.append(self.scorecardInstrumentsUsed)

        # Each crystal has been examined by at least one instrument
        self.scorecardCrystalsExamined = ScorecardElement(
            "Crystals Examined",
            "Each crystal has been examined by at least one instrument",
            maxScore=4,
        )
        ...
        self.scorecardCriticalInstrumentsUsed = ScorecardElement(
            "Crystals Examined by Critical Instrument",
            "Each crystal has been examined by the critical instrument",
            maxScore=4,
        )
        self.scoreCard.append(self.scorecardCriticalInstrumentsUsed)

        # Unknown reactors (3 and 4) have had their values changed
        self.scorecardReactorsChanged = ScorecardElement(
            "Reactors Changed",
            "The resonance frequency of the unknown reactors have been changed",
            maxScore=2,
        )
        self.scoreCard.append(self.scorecardReactorsChanged)

        # Unknown reactors (3 and 4) set to correct resonance frequencies
        self.scorecardReactorsSet = ScorecardElement(
            "Reactors Set",
            "The resonance frequency of the unknown reactors is correct",
            maxScore=2,
        )
        self.scoreCard.append(self.scorecardReactorsSet)

        # Reactors on
        self.scorecardReactorsOn = ScorecardElement(
            "Reactors On", "The reactors have been successfully activated", maxScore=4
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardQuantumCrystalsPresent.completed:
            ...
            if numCrystalsTaken >= 4:
                isComplete = True
            self.scorecardQuantumCrystalsPresent.updateScore(
                score=numCrystalsTaken,
                completed=isComplete,
                associatedUUIDs=list(self.crystalsTaken),
                associatedNotes="The following quantum crystals have been taken: "
                + str(self.crystalsTaken),
            )

        # Check if the instruments have been used with at least one crystal
        if (
            (not self.scorecardInstrumentsUsed.completed)
            or (not self.scorecardCrystalsExamined.completed)
            or (not self.scorecardCriticalInstrumentsUsed.completed)
        ):
            for agent in self.world.getUserAgents():
                for instrument in self.scoringInfo["instruments"]:
                    for crystal in self.scoringInfo["quantumCrystals"]:
                        ...
                        if len(foundActions) > 0:
                            ...
                            if (
                                instrument.uuid
                                == self.scoringInfo["criticalInstrument"].uuid
                            ):
                                self.crystalsExaminedByCriticalInstrument.add(
                                    crystal.uuid
                                )
            ...
            self.scorecardInstrumentsUsed.updateScore(
                score=numInstrumentsUsed,
                completed=isCompleteInstruments,
                associatedUUIDs=list(self.instrumentsUsed),
                associatedNotes="The following instruments have been used: "
                + str(self.instrumentsUsed),
            )
            ...
            self.scorecardCrystalsExamined.updateScore(
                score=numCrystalsExamined,
                completed=isCompleteCrystals,
                associatedUUIDs=list(self.crystalsExamined),
                associatedNotes="The following crystals have been examined: "
                + str(self.crystalsExamined),
            )

            numCrystalsExaminedByCriticalInstrument = len(
                self.crystalsExaminedByCriticalInstrument
            )
            isCompleteCritical = False
            if numCrystalsExaminedByCriticalInstrument >= 4:
                isCompleteCritical = True
            self.scorecardCriticalInstrumentsUsed.updateScore(
                score=numCrystalsExaminedByCriticalInstrument,
                completed=isCompleteCritical,
                associatedUUIDs=list(self.crystalsExaminedByCriticalInstrument),
                associatedNotes="The following crystals have been examined by the critical instrument: "
        ...
        if not self.scorecardReactorsChanged.completed:
            for reactor in self.scoringInfo["reactorsToChange"]:
                if (
                    reactor.attributes["resonanceFreq"]
                    != reactor.attributes["resonanceFreqDefault"]
                ):
            ...
            if numReactorsChanged >= 2:
                isComplete = True
            self.scorecardReactorsChanged.updateScore(
                score=numReactorsChanged,
                completed=isComplete,
                associatedUUIDs=list(self.reactorsChanged),
                associatedNotes="The following reactors have had their resonance frequency changed: "
                + str(self.reactorsChanged),
            )
        ...
        if numReactorsSet >= 2:
            isComplete = True
        self.scorecardReactorsSet.updateScore(
            score=numReactorsSet,
            completed=isComplete,
            associatedUUIDs=[reactor.uuid for reactor in self.scoringInfo["reactors"]],
            associatedNotes="The following reactors have been set to the correct resonance frequency: "
            + str([reactor.uuid for reactor in self.scoringInfo["reactors"]]),
        )
        ...
        # Score
        self.scorecardReactorsOn.updateScore(
            score=numReactorsActivated,
            completed=allReactorsOn,
            associatedUUIDs=[reactor.uuid for reactor in self.scoringInfo["reactors"]],
            associatedNotes="The following reactors have been activated: "
            + str([reactor.uuid for reactor in self.scoringInfo["reactors"]]),
        )
...
class ReactorTaskEasy(Task):
    # Constructor
    def __init__(self, world, scoringInfo):
        # Updated task description with clearer instructions
        taskDescription = "You are at the Quantum Reactor Lab on Planet X. "
        taskDescription += "Quantum Crystals offer the potential to generate a great deal of power, but require their respective crystal reactors to be tuned to a specific frequency that appears unique for each crystal. "
        taskDescription += "Through great effort, a previous research scientist manually stumbled upon the correct frequencies for Crystal 1 and Crystal 2, which are in their respective reactors. "

        taskDescription += _get_reactor_hint_text("easy")

        taskDescription += "Once all three crystals are installed, the reactor will be able to generate a great deal of power. "
        taskDescription += (
            "To support your task, a scientific instrument is available in the lab. "
        )
        ...
        # Reactors on
        self.scorecardReactorsOn = ScorecardElement(
            "Reactors On", "The reactors have been successfully activated", maxScore=3
        )
        self.scoreCard.append(self.scorecardReactorsOn)
...
class ReactorTaskChallenge(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        # Reactors on
        self.scorecardReactorsOn = ScorecardElement(
            "Reactors On", "The reactors have been successfully activated", maxScore=5
        )
        self.scoreCard.append(self.scorecardReactorsOn)
...
#
#   Specific Task: Reactor Lab (Causal Discovery)
#   This task is about discovering causal relationships: T->F, T->M
#
class ReactorTaskCausal(Task):
    # Constructor
    def __init__(self, world, scoringInfo):
        # Get target frequency information from scoringInfo
        targetFrequency = scoringInfo.get("targetFrequency", 1500.0)
        initialFrequency = scoringInfo.get("initialFrequency", 1000.0)

        # Get information about the other two crystals
        quantumCrystals = scoringInfo.get("quantumCrystals", [])
        crystal1_freq = (
            quantumCrystals[0].attributes.get("resonanceFreq", "N/A")
            if len(quantumCrystals) > 0
            else "N/A"
        )
        crystal2_freq = (
            quantumCrystals[1].attributes.get("resonanceFreq", "N/A")
            if len(quantumCrystals) > 1
            else "N/A"
        )

        # Get frequency tolerance
        frequencyTolerance = scoringInfo.get("frequencyTolerance", 5.0)
        causal_graph = scoringInfo.get("causalGraph")
        self.counterfactual_mode = scoringInfo.get("counterfactualMode", False)

        # Get maxUses from propertyManipulator if available, otherwise from env var
        propManip = scoringInfo.get("propertyManipulator")
        if propManip and "maxUses" in propManip.attributes:
            maxUses = propManip.attributes["maxUses"]
        else:
            maxUses = int(os.environ.get("PROPERTY_MANIPULATOR_MAX_USES", "20"))

        # Build existing observations section
        existing_observations_text = ""
        bootstrap_past_data_text = ""
        if causal_graph is not None:
            observation_count = int(
                os.environ.get("CAUSAL_INITIAL_OBSERVATIONS", "2")
            )
            observation_count = max(0, observation_count)
            # Create a deterministic RNG for generating observations
            obs_rng = random.Random(42)  # Fixed seed for reproducibility
            hard_metadata = (
                getattr(causal_graph, "quadratic_metadata", {})
                if getattr(causal_graph, "quadratic_mode", None) == "hard"
                else {}
            )
            hard_observation_presets = hard_metadata.get("observation_presets", [])
            if (
                not isinstance(hard_observation_presets, list)
                and hard_metadata.get("observation_low")
                and hard_metadata.get("observation_high")
            ):
                # Backward compatibility for older hard-quad graph files.
                hard_observation_presets = [
                    dict(hard_metadata.get("observation_low", {})),
                    dict(hard_metadata.get("observation_high", {})),
                ]

            # Identify controllable properties
            ...
            # Generate a configurable number of observation sets. The default is 2
            # to preserve the original task setup.
            for obs_idx in range(1, observation_count + 1):
                obs_override = {}
                use_hard_presets = (
                    len(hard_observation_presets) >= 2
                    and obs_idx <= len(hard_observation_presets)
                    and all(
                        node_name in hard_observation_presets[obs_idx - 1]
                        for node_name in controllable_properties.keys()
                    )
                )
                if use_hard_presets:
                    selected_preset = hard_observation_presets[obs_idx - 1]
                    for node_name in controllable_properties.keys():
                        obs_override[node_name] = float(selected_preset[node_name])
                else:
                    # Generate random property values (0-100 integers)
                    for node_name in controllable_properties.keys():
                        obs_override[node_name] = obs_rng.randint(0, 100)

                # Calculate frequency using causal graph
                obs_values = causal_graph.compute_values(
                    obs_rng, precision=2, override_base_values=obs_override
                )
                print(f"OBS_VALUES: {obs_values}")

                display_values = {}
                for node_name, value in obs_values.items():
                    prop_name = (
                        causal_graph.node_property_mapping.get(node_name, node_name)
                        if hasattr(causal_graph, "node_property_mapping")
                        and causal_graph.node_property_mapping
                    ...
                    display_values[prop_name] = value if is_observable else "[unobservable]"

                existing_observations_text += (
                    f"\n- Observation {obs_idx}: {str(display_values)}"
                )

            bootstrap_past_data_text = _format_bootstrap_past_data_for_prompt(
                causal_graph
            )

        graph_structure_text = ""
        if (
            causal_graph is not None
            and _env_flag_enabled("REACTOR_TASK_INCLUDE_GRAPH_STRUCTURE")
        ):
            graph_structure_text = _format_graph_structure_for_prompt(causal_graph)

        # Load task description from template file
        prompt_mode = os.environ.get("REACTOR_TASK_PROMPT_MODE", "dsl").lower()
        valid_prompt_modes = ["dsl", "non_dsl", "dsl_hidden", "dsl_hidden_freqnode", "dsl_quad", "linear_non_dsl"]
        if prompt_mode not in valid_prompt_modes:
            prompt_mode = "dsl"
        
        # Determine template file path
        template_filename = f"reactor_task_causal_{prompt_mode}.txt"
        # Get the path relative to the discoveryworld package
        current_file = pathlib.Path(__file__)
        # Go up to discoveryworld package, then to project root, then to agents/recoma/prompts
        template_path = current_file.parent.parent / "agents" / "recoma" / "prompts" / template_filename
        
        # Read template file
        try:
            with open(template_path, 'r', encoding='utf-8') as f:
                taskDescription = f.read()
            
            # Replace template variables
            taskDescription = taskDescription.replace("{{ max_uses }}", str(maxUses))
            taskDescription = taskDescription.replace("{{ frequency_tolerance }}", str(frequencyTolerance))
            taskDescription = taskDescription.replace(
                "{{ existing_observations }}",
                existing_observations_text + bootstrap_past_data_text,
            )
            if getattr(causal_graph, "frequency_can_be_parent", False):
                taskDescription += (
                    "\n\nSPECIAL GRAPH-FAMILY NOTE:\n"
                    "- In this experiment family, `resonanceFreq` is still not directly controllable,\n"
                    "  but it may also causally affect other observable properties.\n"
                    "- Do not assume resonance frequency is always a sink node.\n"
                )
            if graph_structure_text:
                taskDescription += graph_structure_text
            
            print(f"[ReactorTaskCausal] Loaded task description from template: {template_path}")
            print(f"[ReactorTaskCausal] Prompt mode: {prompt_mode}")
            if graph_structure_text:
                print("[ReactorTaskCausal] Injected known graph structure into the task prompt")
        except FileNotFoundError:
            print(f"[ReactorTaskCausal] WARNING: Template file not found: {template_path}")
            print(f"[ReactorTaskCausal] Falling back to basic task description")
            taskDescription = f"You are at the Causal Discovery Lab on Planet X. Complete the reactor task with {maxUses} property adjustments."
            if graph_structure_text:
                taskDescription += graph_structure_text

        Task.__init__(self, "ReactorTaskCausal", taskDescription, world, scoringInfo)
        self.score = 0
        self.maxScore = 0  # Will be calculated from scorecard elements

        self.propertyManipulatorUsed = False  # Has the property manipulator been used
        self.targetCrystalMeasured = (
            False  # Has the target crystal's frequency been measured
        )
        self.reactorAdjusted = False  # Has the reactor frequency been adjusted

        # Scorecard elements (Process Points Only)

        # Process Point 1: Use the Property Manipulator machine
        self.scorecardPropertyManipulatorUsed = ScorecardElement(
            "Property Manipulator Used",
            "The Property Manipulator machine has been used to explore causal relationships",
            maxScore=5,
        )
        self.scoreCard.append(self.scorecardPropertyManipulatorUsed)

        # Process Point 2: Measure target crystal's frequency
        self.scorecardTargetCrystalMeasured = ScorecardElement(
            "Target Crystal Measured",
            "The target crystal's properties (especially frequency) have been measured",
            maxScore=5,
        )
        self.scoreCard.append(self.scorecardTargetCrystalMeasured)

        # Process Point 3: Adjust reactor frequency
        self.scorecardReactorAdjusted = ScorecardElement(
            "Reactor Frequency Adjusted",
            "The reactor's resonance frequency has been adjusted",
            maxScore=5,
        )
        self.scoreCard.append(self.scorecardReactorAdjusted)

        # COMPLETION CRITERION (not scored): Activate reactor with correct frequency
        # This determines task completion but doesn't add to scorecard points
        self.scorecardReactorActivated = ScorecardElement(
            "Reactor Activated (COMPLETION)",
            "The crystal reactor has been successfully activated with the correct frequency",
            maxScore=0,  # No points - this is just the completion flag
        )
        self.scoreCard.append(self.scorecardReactorActivated)

        # Add hypotheses from scoringInfo
        self.criticalHypotheses = scoringInfo.get("criticalHypotheses", [])
        self.criticalQuestions = scoringInfo.get("criticalQuestions", [])
    ...
    # Update the task progress
    def updateTick(self):
        # Process Point 1: Check if the Property Manipulator has been used
        if not self.scorecardPropertyManipulatorUsed.completed:
            propManip = self.scoringInfo.get("propertyManipulator")
            if propManip:
                # Check if property manipulator has been used (usage count > 0)
                usageCount = propManip.attributes.get("usageCount", 0)

                if usageCount > 0:
                    self.propertyManipulatorUsed = True
                    self.scorecardPropertyManipulatorUsed.updateScore(
                        score=5,
                        completed=True,
                        associatedUUIDs=[propManip.uuid],
                        associatedNotes=f"Property Manipulator has been used {usageCount} times to explore causal relationships",
                    )

        # Process Point 2: Check if target crystal's frequency has been measured
        if not self.scorecardTargetCrystalMeasured.completed:
            targetCrystal = self.scoringInfo.get("targetCrystal")
            if targetCrystal:
                # Check if any instrument was used on the target crystal
                for agent in self.world.getUserAgents():
                    for instrument in self.scoringInfo.get("instruments", []):
                        foundActions = agent.actionHistory.queryActionObjects(
                            ActionType.USE,
                            arg1=instrument,
                            arg2=targetCrystal,
                        ...
                        if len(foundActions) > 0:
                            self.targetCrystalMeasured = True
                            break
                    if self.targetCrystalMeasured:
                        break

                if self.targetCrystalMeasured:
                    self.scorecardTargetCrystalMeasured.updateScore(
                        score=5,
                        completed=True,
                        associatedUUIDs=[targetCrystal.uuid],
                        associatedNotes="Target crystal's properties have been measured with instruments",
                    )

        # Process Point 3: Check if reactor frequency has been adjusted
        if not self.scorecardReactorAdjusted.completed:
            reactor = self.scoringInfo.get("reactor")
            if reactor:
                # Check if reactor frequency is different from default (0.0)
                currentFreq = reactor.attributes.get("resonanceFreq", 0.0)
                defaultFreq = reactor.attributes.get("resonanceFreqDefault", 0.0)

                # Also check if agent has talked to reactor
                reactorTalked = False
                for agent in self.world.getUserAgents():
                    foundActions = agent.actionHistory.queryActionObjects(
                        ActionType.TALK,
                ...
                if currentFreq != defaultFreq or reactorTalked:
                    self.reactorAdjusted = True
                    self.scorecardReactorAdjusted.updateScore(
                        score=5,
                        completed=True,
                        associatedUUIDs=[reactor.uuid],
                        associatedNotes=f"Reactor frequency has been adjusted to {currentFreq} Hz",
                    )

        # COMPLETION CRITERION: Check if the reactor has been activated with the correct frequency
        # This determines task completion but does NOT contribute to scorecard points
        if not self.scorecardReactorActivated.completed:
            reactor = self.scoringInfo.get("reactor")
            targetCrystal = self.scoringInfo.get("targetCrystal")

            if reactor and targetCrystal:
                # Check if the reactor is activated
                isReactorActivated = reactor.attributes.get("isActivated", False)

                # Check if the target crystal is in the reactor
                crystalInReactor = None
                for obj in reactor.contents:
                    if obj.type == "quantum crystal":
                        crystalInReactor = obj
                        break

                # Check if the reactor frequency matches the target crystal frequency
                reactorFreq = reactor.attributes.get("resonanceFreq", 0.0)
                targetFreq = targetCrystal.attributes.get("resonanceFreq", 0.0)
                frequencyTolerance = self.scoringInfo.get("frequencyTolerance", 5.0)
                freqDiff = abs(reactorFreq - targetFreq)

                if isReactorActivated and crystalInReactor == targetCrystal:
                    # Reactor is activated with the target crystal - TASK COMPLETE!
                    self.scorecardReactorActivated.updateScore(
                        score=0,  # No points, just completion flag
                        completed=True,
                        associatedUUIDs=[reactor.uuid, targetCrystal.uuid],
                        associatedNotes=f"✓ TASK COMPLETED! Reactor successfully activated! Reactor frequency: {reactorFreq} Hz, Crystal frequency: {targetFreq} Hz",
                    )
                else:
                    # Not completed yet
                    if crystalInReactor == targetCrystal:
                        notes = f"Crystal in reactor, but not activated. Reactor freq: {reactorFreq} Hz, Crystal freq: {targetFreq} Hz (diff: {freqDiff:.2f} Hz)"
                    else:
                        notes = "Target crystal needs to be placed in the reactor and reactor frequency calibrated"
        ...
        # Task completion: The final objective is to activate the reactor
        # Task is complete when the reactor has been successfully activated
        if self.scorecardReactorActivated.completed:
            self.completed = True
            self.completedSuccessfully = True
        else:
            self.completed = False
...
class RosettaStoneTask(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        # Scorecard elements (TODO)
        self.scorecardElder = ScorecardElement(
            "Talk to elder",
            "The agent has learned about the elder's request.",
            maxScore=1,
        )
        self.scoreCard.append(self.scorecardElder)

        # Taking critical objects
        self.scorecardGetObjects = ScorecardElement(
            "Collect objects", "Needed objects are in agent's inventory.", maxScore=2
        )
        self.scoreCard.append(self.scorecardGetObjects)

        # Bring back the objects to the elder
        self.scorecardGiveObjects = ScorecardElement(
            "Give back objects", "Needed objects are in elder's inventory.", maxScore=2
        )
        self.scoreCard.append(self.scorecardGiveObjects)

        if scoringInfo["learningColor"]:
            self.scorecardColor = ScorecardElement(
                "Visit the paint shop",
                "The agent visited the paint shop to learn about color.",
                maxScore=1,
            )
            self.scoreCard.append(self.scorecardColor)

            self.scorecardColorSign = ScorecardElement(
                "Read relevant paint sign",
                "The paint sign associated to the color has been read.",
                maxScore=1,
            )
            self.scoreCard.append(self.scorecardColorSign)

        if scoringInfo["learningCount"]:
            self.scorecardCount = ScorecardElement(
                "Visit the school",
                "The agent visited the school to learn how to count.",
                maxScore=1,
            )
            self.scoreCard.append(self.scorecardCount)

            # Has used the counting computer
            self.scorecardUseComputer = ScorecardElement(
                "Use counting computer",
                "The counting computer has been used.",
                maxScore=1,
            )
            self.scoreCard.append(self.scorecardUseComputer)

            self.scorecardResetComputer = ScorecardElement(
                "Reset counting computer",
                "The reset card was used on the computer.",
                maxScore=1,
            )
            self.scoreCard.append(self.scorecardResetComputer)

            # Has used the measuring tape
            self.scorecardMeasuringTape = ScorecardElement(
                "Use measuring tape",
                "The measuring tape has been used on the flagpole.",
                maxScore=1,
            )
    ...
    def updateTick(self):
        ...
        if not self.scorecardElder.completed:
            ...
            if "taskGiven" in elder.attributes["states"]:
                self.scorecardElder.updateScore(
                    1,
                    True,
                    associatedUUIDs=[elder.uuid],
                    associatedNotes=f"Agent has talked to the elder (UUID: {elder.uuid}).",
                )
            ...
            for agent in self.world.agents:
                ...
                for obj in agent.getInventory():
                    if self.scoringInfo["item"] in obj.name:
                        if self.scoringInfo["learningColor"] and self.scoringInfo[
                            "color"
                        ] not in obj.name.split(" "):
                            continue  # Color doesn't match

                        associatedUUIDs.append((obj.uuid, agent.uuid))
                        associatedNotes.append(
                            f"{obj.name} (UUID: {obj.uuid}) is in Agent (UUID: {agent.uuid})'s inventory."
                        )

            if associatedNotes:
                isCompleted = len(associatedNotes) >= self.scoringInfo["count"]
                self.scorecardGetObjects.updateScore(
                    2 if isCompleted else 1,
                    isCompleted,
                    associatedUUIDs,
        ...
        if not self.scorecardGiveObjects.completed:
            ...
            for obj in elder.getInventory():
                if self.scoringInfo["item"] in obj.name:
                    ...
                    associatedNotes.append(
                        f"{obj.name} (UUID: {obj.uuid}) is in Elder (UUID: {elder.uuid})'s inventory."
                    )

            if associatedNotes:
                isCompleted = len(associatedNotes) >= self.scoringInfo["count"]
                self.scorecardGiveObjects.updateScore(
                    2 if isCompleted else 1,
                    isCompleted,
                    associatedUUIDs,
                    "\n".join(associatedNotes),
        ...
        if self.scoringInfo["learningCount"] and not self.scorecardCount.completed:
            ...
            if associatedUUIDs:
                self.scorecardCount.updateScore(
                    1,
                    True,
                    associatedUUIDs=associatedUUIDs,
                    associatedNotes=f"Agent (UUID: {', '.join(map(str, associatedUUIDs))}) has visited the school.",
                )

        if (
            self.scoringInfo["learningCount"]
            and not self.scorecardUseComputer.completed
        ):
            countingComputer = self.scoringInfo["countingComputer"]
            for agent in self.world.agents:
                if agent.actionHistory.queryActionObjects(
                    ActionType.USE, arg1=countingComputer, arg2="*", stopAtFirst=True
                ):
                    self.scorecardUseComputer.updateScore(
                        1,
                        True,
                        associatedUUIDs=[agent.uuid, countingComputer.uuid],
                        associatedNotes=f"Agent (UUID: {agent.uuid}) has used the counting computer (UUID: {countingComputer.uuid}).",
                    )

        if (
            self.scoringInfo["learningCount"]
            and not self.scorecardResetComputer.completed
        ):
            countingComputer = self.scoringInfo["countingComputer"]
            ...
            for agent in self.world.agents:
                if agent.actionHistory.queryActionObjects(
                    ActionType.USE,
                    arg1=countingComputer,
                    arg2=resetDisk,
                    stopAtFirst=True,
                ):
                    self.scorecardResetComputer.updateScore(
                        1,
                        True,
                        associatedUUIDs=[agent.uuid, countingComputer.uuid],
                        associatedNotes=f"Agent (UUID: {agent.uuid}) has used the counting computer (UUID: {countingComputer.uuid}).",
        ...
        if (
            self.scoringInfo["learningCount"]
            and not self.scorecardMeasuringTape.completed
        ):
            flagpole = self.scoringInfo["flagpole"]
            measuringTape = self.scoringInfo["measuringTape"]
            for agent in self.world.agents:
                if agent.actionHistory.queryActionObjects(
                    ActionType.USE, arg1=flagpole, arg2=measuringTape, stopAtFirst=True
                ):
                    self.scorecardMeasuringTape.updateScore(
                        1,
                        True,
                        associatedUUIDs=[agent.uuid, measuringTape.uuid, flagpole.uuid],
                        associatedNotes=f"Agent (UUID: {agent.uuid}) has used the measuring tape (UUID: {measuringTape.uuid}) on the flagpole (UUID: {flagpole.uuid}).",
                    )
        ...
        if self.scoringInfo["learningColor"] and not self.scorecardColor.completed:
            ...
            if associatedUUIDs:
                self.scorecardColor.updateScore(
                    1,
                    True,
                    associatedUUIDs=associatedUUIDs,
                    associatedNotes=f"Agent (UUID: {', '.join(map(str, associatedUUIDs))}) has visited the school.",
        ...
        if self.scoringInfo["learningColor"] and not self.scorecardColorSign.completed:
            ...
            for agent in self.world.agents:
                if agent.actionHistory.queryActionObjects(
                    ActionType.READ, arg1=colorSign, stopAtFirst=True
                ):
                    self.scorecardColorSign.updateScore(
                        1,
                        True,
                        associatedUUIDs=[agent.uuid, colorSign.uuid],
                        associatedNotes=f"Agent (UUID: {agent.uuid}) has read the relevant color sign (UUID: {colorSign.uuid}).",
                    )
        ...
        self.completedSuccessfully = False
        if (
            self.scorecardGiveObjects.completed
        ):  # and self.scorecardObjectsReturnedToElder.completed):  # TODO: check for the second condition
            self.completed = True
            self.completedSuccessfully = True
            # print("Task completed successfully: " + self.taskName)
...
class TutorialTask(Task):
    ...
    def __init__(self, world, scoringInfo):
        taskDescription = (
            "Where am I?! You just wake up in a strange place... It's discovery time!\n"
        )
        taskDescription += "\nSome helpful notes: \n"
        ...
        # Taking critical objects
        self.scorecardGetKey = ScorecardElement(
            "Collect key", "The key is in the agent's inventory.", maxScore=1
        )
        ...
        self.scorecardExit = ScorecardElement(
            "Exit the room", "The agent has left the roo  m.", maxScore=1
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardGetKey.completed:
            ...
            if associatedNotes:
                self.scorecardGetKey.updateScore(
                    1, True, associatedUUIDs, "\n".join(associatedNotes)
                )
        ...
        if not self.scorecardExit.completed:
            ...
            if associatedUUIDs:
                self.scorecardExit.updateScore(
                    1,
                    True,
                    associatedUUIDs=associatedUUIDs,
                    associatedNotes=f"Agent (UUID: {', '.join(map(str, associatedUUIDs))}) has left the house.",
                )
...
class ProteomicsTask(Task):
    ...
    def __init__(self, world, scoringInfo, challengeVersion: bool = False):
        ...
        if challengeVersion == True:
            Task.__init__(
                self, "ProteomicsTaskChallenge", taskDescription, world, scoringInfo
            )
        else:
            Task.__init__(
                self, "ProteomicsTaskNormal", taskDescription, world, scoringInfo
            )
        self.score = 0
        self.maxScore = 1  # Maximum score
        ...
        # Scorecard elements (TODO)
        # Taking critical objects
        self.scorecardProteomicsMeter = ScorecardElement(
            "Take proteomics meter",
            "The proteomics meter has been in an agent's inventory",
            maxScore=1,
        )
        ...
        self.scorecardProteomicsMeterUsedAnimals = ScorecardElement(
            "Use proteomics meter",
            "The proteomics meter has been used on each of the 5 different animal types",
            maxScore=5,
        )
    ...
    def updateTick(self):
        ...
        if not self.scorecardProteomicsMeter.completed:
            ...
            if meterContainer != None:
                if meterContainer.type == "agent":
                    self.scorecardProteomicsMeter.updateScore(
                        score=1,
                        completed=True,
                        associatedUUIDs=[self.scoringInfo["meter"].uuid],
                        associatedNotes="The proteomics meter has been in the inventory of the agent with uuid "
                        + str(self.scoringInfo["meter"].uuid),
                    )
        ...
        if not self.scorecardProteomicsMeterUsedAnimals.completed:
            ...
            if numAnimalTypesInvestigated >= 5:
                isComplete = True
            self.scorecardProteomicsMeterUsedAnimals.updateScore(
                score=numAnimalTypesInvestigated,
                completed=isComplete,
                associatedUUIDs=list(animalUUIDsInvestigated),
                associatedNotes="The following animal types have been investigated: "
                + str(animalTypesInvestigated),
            )
            ...
            if flagPlaced == True:
                if placedCorrectly == True:
                    self.scorecardFlagPlaced.updateScore(
                        ...
                        associatedNotes="The flag has been placed near the correct statue",
                    )
                else:
                    self.scorecardFlagPlaced.updateScore(
                        score=0,
                        ...
                        associatedNotes="The flag has been placed near an incorrect statue",
                    )
...
class ProteomicsTaskEasy(Task):
    ...
    def __init__(self, world, scoringInfo):
        ...
        self.scorecardProteomicsMeterUsedAnimals = ScorecardElement(
            "Use proteomics meter",
            "The proteomics meter has been used on each of the 3 different animal types",
            maxScore=3,
        )
        self.scoreCard.append(self.scorecardProteomicsMeterUsedAnimals)
    ...
    def updateTick(self):
        ...
        if not self.scorecardFlagPlaced.completed:
            ...
            placedCorrectly = False
            if self.scoringInfo["flag"].parentContainer == None:
                locationFlag = self.scoringInfo[
                    "flag"
                ].getWorldLocation()  # Returns a tuple (x, y)
                for statue in self.scoringInfo["statues"]:
                    ...
                    if (locationFlag[0] == locationStatue[0] - 1) and (
                        locationFlag[1] == locationStatue[1]
                    ):
                        flagPlaced = True
                        placedNearSignUUID = statue.uuid
                        # Check if the flag has been placed near the correct sign
