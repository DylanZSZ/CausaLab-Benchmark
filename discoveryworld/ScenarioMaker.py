...
SCENARIO_INFOS = {
    ...
    "Reactor Lab Causal": {
        "difficulty": ["Causal"],
        "variations": [str(i) for i in range(1, 65)],
    },
    "Lost in Translation": {
        "difficulty": ["Easy", "Normal", "Challenge"],
        "variations": ["1", "2", "3", "4", "5"],
...
def getInternalScenarioName(scenarioNameIn:str, difficulty:str):
    ...
    # Scenario 4b: Reactor Lab Causal (with configurable causal parameters)
    elif (scenarioNameIn == "Reactor Lab Causal") and (difficulty == "Causal"):
        scenarioName = "reactor_lab_causal"
...
class ScenarioMaker():
    ...
    # Make a scenario
    def setupScenario(self, scenarioName:str, numUserAgents:int=1, causal_config=None):
        ...
        elif (scenarioName == "reactor_lab_causal"):
            scoringInfo = makeScenarioReactorLabCausal(self.world, numUserAgents, config=causal_config)
            self.world.addTaskByName("ReactorTaskCausal", scoringInfo)
            self.world.initialFilter()
            return (True, "")
