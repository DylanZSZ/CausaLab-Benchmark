...
from discoveryworld.property_manipulator_oracle import (
    build_maximal_changed_paths,
    format_oracle_chain_strings,
)
...
class Agent(Object):
    ...
    def __init__(
        ...
        objectType="agent",
        objectName="agent",
        defaultSpriteName="character18_agent_facing_south",
    ):
        # Default sprite name
        super().__init__(
            world, objectType, objectName, defaultSpriteName=defaultSpriteName
        )

        # Rendering
        self.attributes["faceDirection"] = "south"
        self.spriteCharacterPrefix = (
            "character18_"  # Prefix for the sprite character name (e.g. "character18_")
        )
        ...
        self.attributes["isOpenContainer"] = (
            True  # If it's a container, then is it open?
        )
        self.attributes["containerPrefix"] = (
            "on"  # Container prefix (e.g. "in" or "on")
        )
        self.attributes["contentsVisible2D"] = (
            False  # If it is a container, do we render the contents in the 2D representation, or is that already handled (e.g. for pots/jars, that render generic contents if they contain any objects)
        )

        # Dialog attributes
        self.attributes["isDialogable"] = True  # Can it be dialoged with?
        self.attributes["inDialogWith"] = (
            None  # Who is it in dialog with at this current moment?
        )
        self.dialogTree = None  # Dialog tree for this agent

        # Door opening/closing for NPCs
        self.attributes["doorNeedsToBeClosed"] = (
            None  # Whether a door was recently opened that needs to be closed
        )
        self.attributes["movesSinceDoorOpen"] = (
            0  # How many moves have happened since the door was opened
        )

        # Signals (largely for NPCs)
        self.attributes["states"] = (
            set()
        )  # External signals that the agent has received

        # Health attributes
        self.attributes["poisonedCounter"] = (
            -1
        )  # Poisoned counter (if >= 0, then the agent is poisoned)
    ...
    def getInventory(self):
        ...
        for obj in self.contents:
            ...
            accessibleInventoryObjects.extend(
                obj.getAllContainedObjectsRecursive(respectContainerStatus=True)
            )
    ...
    def getObjectsAgentFacing(self, respectContainerStatus=True):
        ...
        objsAtAgentLocation = self.world.getObjectsAt(
            self.attributes["gridX"],
            self.attributes["gridY"],
            respectContainerStatus=respectContainerStatus,
            respectObscuringLowerLayers=True,
        )
        ...
        objsInFront = self.world.getObjectsAt(
            facingLocation[0],
            facingLocation[1],
            respectContainerStatus=respectContainerStatus,
            respectObscuringLowerLayers=True,
    ...
    def getValidDirectionsToMoveTo(self):
        ...
        for direction in ["north", "east", "south", "west"]:
            ...
            if (
                newX < 0
                or newX >= self.world.sizeX
                or newY < 0
                or newY >= self.world.sizeY
            ):
                # Invalid location
                continue
    ...
    def getNearbyVisibleObjects(self, maxDistance=2, includeUUID: bool = False):
        visibleObjectsFull = (
            []
        )  # The actual objects rather than dicts representing them, for downstream processing
        visibleObjects = []
        ...
        for x in range(
            self.attributes["gridX"] - maxDistance,
            self.attributes["gridX"] + maxDistance + 1,
        ):
            for y in range(
                self.attributes["gridY"] - maxDistance,
                self.attributes["gridY"] + maxDistance + 1,
            ):
                ...
                if self.world.isWithinBounds(x, y):
                    ...
                    objs = self.world.getObjectsAt(
                        x,
                        y,
                        respectContainerStatus=True,
                        includeParts=False,
                        respectObscuringLowerLayers=True,
                    )
                ...
                for obj in objsAtLocation:
                    ...
                    # Distance
                    distance = abs(
                        obj.attributes["gridX"] - self.attributes["gridX"]
                    ) + abs(obj.attributes["gridY"] - self.attributes["gridY"])
    ...
    def tick(self):
        ...
        if self.attributes["poisonedCounter"] != -1:
            ...
            if self.attributes["poisonedCounter"] < -POISON_INCUBATION_PERIOD:
                self.attributes["poisonedCounter"] = (
                    POISON_DURATION  # Duration of poison
                )
                self.attributes["isPoisoned"] = True
                self.actionDiscoveryFeedMakeUpdatePost(
                    "I'm not feeling well.", signals=[]
                )
    ...
    def actionRotateAgentFacingDirection(self, direction=+1):
        ...
        actionType = (
            ActionType.ROTATE_CW if (direction == +1) else ActionType.ROTATE_CCW
        )
        self.actionHistory.add(
            actionType=actionType, arg1=None, arg2=None, result=result
        )
        return result
    ...
    def actionRotateAgentFacingDirectionAbsolute(self, direction):
        ...
        self.actionHistory.add(
            actionType=ActionType.ROTATE_DIRECTION,
            arg1=direction,
            arg2=None,
            result=result,
        )
    ...
    def actionMoveAgentForwardBackward(self, direction=+1):
        actionType = (
            ActionType.MOVE_FORWARD if (direction == +1) else ActionType.MOVE_BACKWARD
        )
        # Get the current direction
        currentDirection = self.attributes["faceDirection"]
        ...
        if newX < 0 or newX >= self.world.sizeX or newY < 0 or newY >= self.world.sizeY:
            ...
            result = ActionSuccess(
                False, "That would take me beyond the edge of the world."
            )
            self.actionHistory.add(
                actionType=actionType, arg1=None, arg2=None, result=result
            )
        ...
        if not isPassable:
            result = ActionSuccess(
                False,
                "I can't move there. There is something in the way ("
                + blockingObject.name
                + ").",
            )
            self.actionHistory.add(
        ...
        # If we reach here, the new location is valid. Update the agent's location to the new location
        self.world.removeObject(
            self
        )  # First, remove the object from it's current location in the world grid
        self.world.addObject(
            newX, newY, Layer.AGENT, self
        )  # Then, add the object to the new location in the world grid

        # Invalidate sprite name
        self.needsSpriteNameUpdate = True

        result = ActionSuccess(
            True, "I moved to (" + str(newX) + ", " + str(newY) + ")."
        )
        self.actionHistory.add(
            actionType=actionType, arg1=None, arg2=None, result=result
        )
    ...
    def actionMoveAgentNorthEastSouthWest(self, direction):
        ...
        if newX < 0 or newX >= self.world.sizeX or newY < 0 or newY >= self.world.sizeY:
            ...
            self.actionHistory.add(
                actionType=actionType, arg1=direction, arg2=None, result=result
            )
            return result
    ...
    def actionTeleportAgentToLocation(self, locationName):
        ...
        if not locationName in knownTeleportLocations:
            result = ActionSuccess(
                False,
                "That's not a known teleport location ('"
                + str(locationName)
                + "'). I know about the following locations: "
                + str(sorted(list(knownTeleportLocations.keys()))),
            )
            self.actionHistory.add(
                actionType=ActionType.TELEPORT_TO_LOCATION,
                arg1=locationName,
                arg2=None,
                result=result,
            )
        ...
        self.world.addObject(
            location["gridX"], location["gridY"], Layer.AGENT, self
        )  # Then, add the object to the new location in the world grid
    ...
    def actionTeleportAgentToObject(self, objectUUID):
        ...
        try:
            ...
            result = ActionSuccess(
                False,
                "That does not appear to be a valid UUID ("
                + str(objectUUID)
                + ").  UUIDs should be integers.",
            )
            self.actionHistory.add(
                actionType=ActionType.TELEPORT_TO_OBJECT,
                arg1=objectUUID,
                arg2=None,
                result=result,
            )
        ...
        if objToTeleportTo == None:
            result = ActionSuccess(
                False, "No object with that UUID (" + str(objectUUID) + ") was found."
            )
            self.actionHistory.add(
                actionType=ActionType.TELEPORT_TO_OBJECT,
                arg1=objectUUID,
        ...
        newY = -1
        # North
        passableNorth, blockingObjectNorth = self.world.isPassable(
            objLocation[0], objLocation[1] - 1
        )
        passableEast, blockingObjectEast = self.world.isPassable(
            objLocation[0] + 1, objLocation[1]
        )
        passableSouth, blockingObjectSouth = self.world.isPassable(
            objLocation[0], objLocation[1] + 1
        )
        passableWest, blockingObjectWest = self.world.isPassable(
            objLocation[0] - 1, objLocation[1]
        )
        if passableNorth:
            newX = objLocation[0]
        ...
        if newX == -1 or newY == -1:
            result = ActionSuccess(
                False,
                "I couldn't find a valid location beside the object ("
                + str(objToTeleportTo.name)
                + ", uuid: "
                + str(objectUUID)
                + ") to teleport to.",
            )
        ...
        result = ActionSuccess(
            True,
            "I teleported to the object with UUID "
            + str(objectUUID)
            + " ("
            + str(objToTeleportTo.name)
            + ") at ("
    ...
    def actionPickUp(self, objToPickUp):
        ...
        if not objToPickUp.attributes["isMovable"]:
            ...
            result = ActionSuccess(
                False, "That object (" + objToPickUp.name + ") is not movable."
            )
            self.actionHistory.add(
                actionType=ActionType.PICKUP, arg1=objToPickUp, arg2=None, result=result
            )
            return result
        ...
        if distX > 1 or distY > 1:
            ...
            result = ActionSuccess(
                ...
                "That object ("
                + objToPickUp.name
                + ") is not within reach. I can only pick up objects that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=ActionType.PICKUP, arg1=objToPickUp, arg2=None, result=result
            )
    ...
    def actionDrop(self, objToDrop):
        ...
        if not objToDrop in objectsInInventory:
            ...
            result = ActionSuccess(
                False, "That object (" + objToDrop.name + ") is not in my inventory."
            )
            self.actionHistory.add(
                actionType=ActionType.DROP, arg1=objToDrop, arg2=None, result=result
            )
            return result
        ...
        self.world.addObject(
            self.attributes["gridX"], self.attributes["gridY"], Layer.OBJECTS, objToDrop
        )
    ...
    def actionPut(self, objToPut, newContainer, bypassClosedContainerCheck=False):
        ...
        if not objToPut in objectsInInventory:
            # Object is not in the agent's inventory
            result = ActionSuccess(
                False, "That object (" + objToPut.name + ") is not in my inventory."
            )
            self.actionHistory.add(
        ...
        if distX > 1 or distY > 1:
            ...
            result = ActionSuccess(
                ...
                "That container ("
                + newContainer.name
                + ") is not within reach. I can only put objects into containers that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=ActionType.PUT,
                arg1=objToPut,
        ...
        if not newContainer.attributes["isContainer"]:
            ...
            result = ActionSuccess(
                False, "That object (" + newContainer.name + ") is not a container."
            )
            self.actionHistory.add(
                actionType=ActionType.PUT,
                arg1=objToPut,
        ...
        bypassClosedContainerCheck = (
            bypassClosedContainerCheck or newContainer.attributes.get("isNPC")
        )

        # Next, check to see if the container is open
        if (
            not newContainer.attributes["isOpenContainer"]
            and not bypassClosedContainerCheck
        ):
            # Container is not open
            result = ActionSuccess(
                False, "That container (" + newContainer.name + ") is not open."
            )
            self.actionHistory.add(
                actionType=ActionType.PUT,
                arg1=objToPut,
        ...
        if objToPut.uuid == newContainer.uuid:
            ...
            result = ActionSuccess(
                False, "I can't put the " + objToPut.name + " into itself."
            )
            self.actionHistory.add(
                actionType=ActionType.PUT,
                arg1=objToPut,
        ...
        allContainedObjects = objToPut.getAllContainedObjectsAndParts(
            includeContents=True, includeParts=True
        )
        ...
        if newContainer.attributes.get("isNPC"):
            result = ActionSuccess(
                True, "I gave the " + objToPut.name + " to " + newContainer.name + "."
            )
        else:
            result = ActionSuccess(
                True,
                "I put the " + objToPut.name + " into the " + newContainer.name + ".",
            )

        self.actionHistory.add(
            actionType=ActionType.PUT, arg1=objToPut, arg2=newContainer, result=result
        )
        return result
    ...
    def actionThrow(self, objToThrow, throwLocation):
        ...
        if not objToThrow in objectsInInventory:
            # Object is not in the agent's inventory
            result = ActionSuccess(
                False, "That object (" + objToThrow.name + ") is not in my inventory."
            )
            self.actionHistory.add(
                actionType=ActionType.DROP, arg1=objToThrow, arg2=None, result=result
            )
            return result
        ...
        self.world.addObject(
            throwLocation[0], throwLocation[1], Layer.OBJECTS, objToThrow
        )  # TODO: add animation for throwing
    ...
    def actionOpenClose(self, objToOpenOrClose, whichAction="open"):
        ...
        if distX > 1 or distY > 1:
            ...
            result = ActionSuccess(
                False,
                "That object ("
                + objToOpenOrClose.name
                + ") is not within reach. I can only open/close objects that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=actionType, arg1=objToOpenOrClose, arg2=None, result=result
            )
            return result
        ...
        if objToOpenOrClose.attributes["isContainer"]:
            ...
            if whichAction == "open" and objToOpenOrClose.attributes["isOpenContainer"]:
                ...
                result = ActionSuccess(
                    False,
                    "That object (" + objToOpenOrClose.name + ") is already open.",
                )
                self.actionHistory.add(
                    actionType=actionType,
                    arg1=objToOpenOrClose,
                    arg2=None,
                    result=result,
                )
                return result
                ...
                whichAction == "close"
                and not objToOpenOrClose.attributes["isOpenContainer"]
            ):
                # Object is already closed
                result = ActionSuccess(
                    False,
                    "That object (" + objToOpenOrClose.name + ") is already closed.",
                )
                self.actionHistory.add(
                    actionType=actionType,
                    arg1=objToOpenOrClose,
            ...
            if whichAction == "open":
                ...
                result = ActionSuccess(
                    True, "I opened the " + objToOpenOrClose.name + "."
                )
                self.actionHistory.add(
                    actionType=actionType,
                    arg1=objToOpenOrClose,
                ...
                self.updateLastInteractedObject([objToOpenOrClose])
                result = ActionSuccess(
                    True, "I closed the " + objToOpenOrClose.name + "."
                )
                self.actionHistory.add(
                    actionType=actionType,
                    arg1=objToOpenOrClose,
            ...
            if whichAction == "open" and objToOpenOrClose.attributes["isOpenPassage"]:
                ...
                whichAction == "close"
                and not objToOpenOrClose.attributes["isOpenPassage"]
            ):
                # Object is already closed
                result = ActionSuccess(
                    False,
            ...
            if whichAction == "open":
                ...
                if objToOpenOrClose.attributes["requiresKey"] > 0:
                    ...
                    for obj in self.getInventory():
                        if (
                            obj.attributes["keyID"]
                            == objToOpenOrClose.attributes["requiresKey"]
                        ):
                            keyFound = True
                            key = obj
                            break
                    ...
                    if key.attributes["isRusted"]:
                        result = ActionSuccess(
                            False, "The key is rusted and doesn't work."
                        )
                        self.actionHistory.add(
                            actionType=actionType,
                            arg1=objToOpenOrClose,
                ...
                if requiredKey:
                    result = ActionSuccess(
                        True, "I opened the " + objToOpenOrClose.name + " with a key."
                    )
                else:
                    result = ActionSuccess(
                        True, "I opened the " + objToOpenOrClose.name + "."
                ...
                if (
                    self.attributes["gridX"] == objToOpenOrClose.attributes["gridX"]
                    and self.attributes["gridY"] == objToOpenOrClose.attributes["gridY"]
                ):
                    result = ActionSuccess(
                        False,
                        "I can't close the "
    ...
    def actionActivateDeactivate(
        self, objToActivateOrDeactivate, whichAction="activate"
    ):
        actionType = (
            ActionType.ACTIVATE if whichAction == "activate" else ActionType.DEACTIVATE
        )
        # First, check if the object is within reach (i.e. +/- 1 grid location)
        distX = abs(
            objToActivateOrDeactivate.attributes["gridX"] - self.attributes["gridX"]
        )
        distY = abs(
            objToActivateOrDeactivate.attributes["gridY"] - self.attributes["gridY"]
        )
        if distX > 1 or distY > 1:
            # Object is not within reach
            result = ActionSuccess(
                False,
                "That object ("
                + objToActivateOrDeactivate.name
                + ") is not within reach. I can only activate/deactivate objects that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=actionType,
                arg1=objToActivateOrDeactivate,
                arg2=None,
                result=result,
            )
            return result
        ...
        if not objToActivateOrDeactivate.attributes["isActivatable"]:
            ...
            result = ActionSuccess(
                ...
                "That object ("
                + objToActivateOrDeactivate.name
                + ") is not activatable/deactivatable.",
            )
            self.actionHistory.add(
                actionType=actionType,
                arg1=objToActivateOrDeactivate,
        ...
        # Next, check if the object is already in the desired state
        if (
            whichAction == "activate"
            and objToActivateOrDeactivate.attributes["isActivated"]
        ):
            ...
            whichAction == "deactivate"
            and not objToActivateOrDeactivate.attributes["isActivated"]
        ):
        ...
        if whichAction == "activate":
            ...
            result = ActionSuccess(
                True, "I activated the " + objToActivateOrDeactivate.name + "."
            )
            self.actionHistory.add(
                actionType=actionType,
                arg1=objToActivateOrDeactivate,
            ...
            self.updateLastInteractedObject([objToActivateOrDeactivate])
            result = ActionSuccess(
                True, "I deactivated the " + objToActivateOrDeactivate.name + "."
            )
            self.actionHistory.add(
                actionType=actionType,
                arg1=objToActivateOrDeactivate,
    ...
    def actionEat(self, objToEat):
        ...
        if not objToEat.attributes["isEdible"]:
            # Object is not movable
            result = ActionSuccess(
                False, "That object (" + objToEat.name + ") is not edible."
            )
            self.actionHistory.add(
                actionType=ActionType.EAT, arg1=objToEat, arg2=None, result=result
            )
            return result
        ...
        if distX > 1 or distY > 1:
            ...
            result = ActionSuccess(
                ...
                "That object ("
                + objToEat.name
                + ") is not within reach. I can only eat objects that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=ActionType.EAT, arg1=objToEat, arg2=None, result=result
            )
        ...
        for obj in allObjs:
            ...
            for part in objParts:
                ...
                allPartContents = part.getAllContainedObjectsRecursive(
                    respectContainerStatus=False
                )
                # add to objAndParts
    ...
    def actionRead(self, objToRead):
        ...
        if distX > 1 or distY > 1:
            ...
            result = ActionSuccess(
                ...
                "That object ("
                + objToRead.name
                + ") is not within reach. I can only read objects that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=ActionType.READ, arg1=objToRead, arg2=None, result=result
            )
            return result
        ...
        if not objToRead.attributes["isReadable"]:
            ...
            result = ActionSuccess(
                False, "That object (" + objToRead.name + ") is not readable."
            )
            self.actionHistory.add(
                actionType=ActionType.READ, arg1=objToRead, arg2=None, result=result
            )
        ...
        if len(objToRead.attributes["document"].strip()) == 0:
            ...
            result = ActionSuccess(
                False,
                "The " + objToRead.name + " appears blank. There is nothing to read.",
    ...
    def actionUse(self, objToUse, objToUseOn):
        ...
        if distX > 1 or distY > 1:
            ...
            result = ActionSuccess(
                ...
                "That object ("
                + objToUse.name
                + ") is not within reach. I can only use objects that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=ActionType.USE, arg1=objToUse, arg2=objToUseOn, result=result
            )
            return result
        ...
        if not objToUse.attributes["isUsable"]:
            ...
            result = ActionSuccess(
                False, "That object (" + objToUse.name + ") is not usable."
            )
            self.actionHistory.add(
                actionType=ActionType.USE, arg1=objToUse, arg2=objToUseOn, result=result
            )
        ...
        if distX > 1 or distY > 1:
            ...
            result = ActionSuccess(
                ...
                "That object ("
                + objToUseOn.name
                + ") is not within reach. I can only use objects on other objects that are within +/- 1 grid location.",
            )
            self.actionHistory.add(
                actionType=ActionType.USE, arg1=objToUse, arg2=objToUseOn, result=result
            )
    ...
    def actionDiscoveryFeedGetPosts(self, startFromID=None):
        ...
        if startFromID is None:
            ...
            notificationStr = (
                "Showing the most recent " + str(len(lastPosts)) + " posts."
            )
        else:
            ...
            if postLocation >= 0:
                ...
                lastPosts = allPosts[-numPostsToRetrieve:]
                notificationStr = (
                    "Failed to find update posts with ID "
                    + str(startFromID)
                    + ", showing the most recent "
                    + str(len(lastPosts))
                    + " posts."
        ...
        self.actionHistory.add(
            actionType=ActionType.DISCOVERY_FEED_GET_UPDATES,
            arg1=None,
            arg2=None,
            result=result,
        )
    ...
    def actionDiscoveryFeedGetArticleTitles(self, startFromID=0):
        ...
        if startFromID <= 0:
            ...
            notificationStr = (
                "Showing the most recent " + str(len(lastArticles)) + " articles."
            )
        else:
            # Get the last N posts starting from the specified ID.  Posts are sorted in ascending order.
            postLocation = -1
            ...
            if postLocation >= 0:
                ...
                notificationStr = (
                    "Showing the most recent "
                    + str(len(lastArticles))
                    + " articles starting from document ID "
                    + str(startFromID)
                    + "."
                )
            else:
                ...
                lastArticles = allArticles[-numArticlesToRetrieve:]
                notificationStr = (
                    "Failed to find articles with ID "
                    + str(startFromID)
                    + ", showing the most recent "
                    + str(len(lastArticles))
                    + " articles."
        ...
        for post in lastArticles:
            postStr = (
                "Article " + str(post["postID"]) + " Title: " + post["title"] + "\n"
            )
            postStr += (
                "Submitted by "
                + post["author"]
        ...
        # Add to action history
        self.actionHistory.add(
            actionType=ActionType.DISCOVERY_FEED_GET_ARTICLES,
            arg1=None,
            arg2=None,
            result=result,
        )
    ...
    def actionDiscoveryFeedGetByID(self, postID):
        ...
        outStr += (
            "Search results for DiscoveryFeed document ID " + str(postID) + ":\n\n"
        )
        ...
        if post == None:
            ...
            self.actionHistory.add(
                actionType=ActionType.DISCOVERY_FEED_GET_POST_BY_ID,
                arg1=postID,
                arg2=None,
                result=result,
            )
        ...
        if post["type"] == "update":
            ...
            result = ActionSuccess(
                False,
                "Unknown post type: " + str(post["type"]) + "\n" + str(post),
    ...
    def actionDiscoveryFeedMakeUpdatePost(self, contentStr, signals: list = []):
        # Create the post
        postID = self.world.discoveryFeed.addUpdatePost(
            self.world.step, self.name, contentStr, signals
        )
        ...
        # Add to action history
        self.actionHistory.add(
            actionType=ActionType.DISCOVERY_FEED_CREATE_UPDATE,
            arg1=post,
            arg2=None,
            result=result,
        )
    ...
    def actionDiscoveryFeedMakeArticle(self, titleStr, contentStr):
        # Create the article
        postID = self.world.discoveryFeed.addArticle(
            self.world.step, self.name, titleStr, contentStr
        )
        ...
        postStr = "Article " + str(post["postID"]) + ": " + post["title"] + "\n"
        postStr += (
            "Submitted by " + post["author"] + " on Step " + str(post["step"]) + "\n\n"
        )
        postStr += post["content"] + "\n"
        outStr += postStr
        ...
        # Add to action history
        self.actionHistory.add(
            actionType=ActionType.DISCOVERY_FEED_CREATE_ARTICLE,
            arg1=post,
            arg2=None,
            result=result,
        )
    ...
    def rotateToFaceDirection(self, directionToFace):
        ...
        if curDirection == directionToFace:
            return ActionSuccess(
                False,
                "I am already facing in the requested direction ("
                + directionToFace
                + ").",
            )
        ...
        result = ActionSuccess(
            True,
            "I am rotating towards facing the requested direction ("
            + directionToFace
            + ").",
        )
        actionType = (
            ActionType.ROTATE_CW if nextMovement == +1 else ActionType.ROTATE_CCW
        )
        self.actionHistory.add(
            actionType=actionType, arg1=directionToFace, arg2=None, result=result
        )
        return result
    ...
    def convertXYDeltasToDirection(self, deltaX, deltaY):
        if (deltaX == 0) and (deltaY == +1):
            ...
            raise ValueError(
                "Invalid deltaX/deltaY: "
                + str(deltaX)
                + ", "
                + str(deltaY)
                + " (must be (0, +1), (0, -1), (+1, 0), or (-1, 0))"
            )
    ...
    def addAutopilotActionToQueue(self, action):
        ...
        self.autopilotActionQueue.sort(
    ...
    def isInDialog(self):
        return ("inDialogWith" in self.attributes) and (
            self.attributes["inDialogWith"] != None
        )
    ...
    def actionDialog(self, agentToTalkTo, dialogStrToSay=None):
        # Check if dialogable
        if ("isDialogable" in agentToTalkTo.attributes) and (
            agentToTalkTo.attributes["isDialogable"] == False
        ):
            result = ActionSuccess(
                False, "You can't talk to that (" + agentToTalkTo.name + ")."
            )
            self.actionHistory.add(
                actionType=ActionType.TALK, arg1=agentToTalkTo, arg2=None, result=result
            )
            return result

        # Check if the other agent is busy, and we're not already talking to it
        if (agentToTalkTo.isBusyAutopilot()) and (
            agentToTalkTo.dialogTree.getAgentTalkingTo() != self
        ):
            result = ActionSuccess(
                False,
                "That agent (" + agentToTalkTo.name + ") is busy, try again later.",
            )
            self.actionHistory.add(
                actionType=ActionType.TALK, arg1=agentToTalkTo, arg2=None, result=result
            )
        ...
        if agentToTalkTo.dialogTree == None:
            result = ActionSuccess(
                False, "That agent (" + agentToTalkTo.name + ") has no dialog tree set."
            )
            self.actionHistory.add(
                actionType=ActionType.TALK, arg1=agentToTalkTo, arg2=None, result=result
            )
        ...
        if agentToTalkTo.dialogTree.getAgentTalkingTo() == self:
            ...
            if dialogStrToSay == None:
                ...
                result = ActionSuccess(
                    True, "Finished talking to " + str(agentToTalkTo.name) + "."
                )
                self.actionHistory.add(
            ...
            success = agentToTalkTo.dialogTree.say(
                thingToSay=dialogStrToSay, agentEngaging=self
            )
            if success == False:
                agentToTalkTo.dialogTree.endDialog()
                self.setNotInDialog()
                result = ActionSuccess(
                    False,
                    "Something went wrong in the dialog -- the response was unexpected.",
                )
                self.actionHistory.add(
                    actionType=ActionType.TALK,
                    arg1=agentToTalkTo,
            ...
            # If no dialog options available (e.g., reached endNode), exit the dialog
            ...
            if agentToTalkTo.dialogTree.isBusy() == True:
                ...
                result = ActionSuccess(
                    ...
                    + agentToTalkTo.name
                    + ") is busy talking to ("
                    + str(agentToTalkTo.dialogTree.getAgentTalkingTo().name)
                    + ").",
                )
                self.actionHistory.add(
                    actionType=ActionType.TALK,
                ...
                # Get the NPC's response, and our possible next dialog options
                npcResponse, nextDialogOptions = (
                    agentToTalkTo.dialogTree.getCurrentDialog()
                )
...
class NPC(Agent):
    ...
    def __init__(self, world, name, defaultSpriteName="character17_agent_facing_south"):
        ...
        # NPC States
        self.attributes["dialogAgentsSpokenWith"] = (
            []
        )  # List of dialog agents that this NPC has spoken with
    ...
    def _doNPCAutonavigation(self):
        ...
        pathSuccess, nextX, nextY, pathLength = self.pathfinder.findPathNextStep(
            self.world,
            self.attributes["gridX"],
            self.attributes["gridY"],
            self.attributes["goalLocation"][0],
            self.attributes["goalLocation"][1],
        )
        ...
        if (
            ("doorNeedsToBeClosed" in self.attributes)
            and (self.attributes["doorNeedsToBeClosed"] != None)
            and (self.attributes["movesSinceDoorOpen"] == 1)
        ):
            ...
            if desiredDirection != self.attributes["faceDirection"]:
                ...
                rotateSuccess = self.actionRotateAgentFacingDirectionAbsolute(
                    desiredDirection
                )
                return True
            ...
            allObjsNotPassable = [
                obj for obj in allObjs if (obj.attributes["isPassable"] == False)
            ]
...
class NPCChef(NPC):
    ...
    def __init__(self, world, name):
        ...
        Agent.__init__(
            ...
            "agent",
            name,
            defaultSpriteName="character17_agent_facing_south",
            tables=None,
            pot=None,
        )
        ...
        if "goalLocation" in self.attributes:
            ...
            if not success:
                ...
                if "pickupFoodFromCafeteria" in self.attributes["states"]:
                    if (
                        self.attributes["gridX"] == self.attributes["goalLocation"][0]
                    ) and (
                        self.attributes["gridY"] == self.attributes["goalLocation"][1]
                    ):
                    ...
                    self.attributes["goalLocation"] = (
                        random.randint(0, self.world.sizeX - 1),
                        random.randint(0, self.world.sizeY - 1),
                    )
...
class NPCColonist(NPC):
    ...
    def __init__(self, world, name):
        ...
        Agent.__init__(
            ...
            "agent",
            name,
            defaultSpriteName="character16_agent_facing_south",
        )
    ...
    def tick(self):
        ...
        if "poisoned" in self.attributes["states"]:
            ...
            edibleObjects = [
                x for x in objectsInFrontOfAgent if x.attributes["isEdible"]
            ]
            # Print names of edible objects in front of agent
...
class NPCColonist1(NPC):
    # Constructor
    def __init__(
        self, world, name, thingsToPickup=None, whereToPlace=None
    ):  ## DEBUG: thingToPickUp is a placeholder
        # Default sprite name
        Agent.__init__(
            self,
            world,
            "agent",
            name,
            defaultSpriteName="character15_agent_facing_south",
        )
        ...
        self.addAutopilotActionToQueue(
            AutopilotAction_PickupObjectsInArea(
                fieldX, fieldY, fieldWidth, fieldHeight, objectTypes, container
            )
        )
    ...
    def tick(self):
        ...
        if (len(self.autopilotActionQueue) > 0) and (
            self.attributes["inDialogWith"] == None
        ):
            ...
            result = self.pathfinder.actionInterpreter(
                curAutopilotAction, agent=self, world=self.world
            )
...
class NPCChef1(NPC):
    ...
    def tick(self):
        ...
        if "poisoned" in self.attributes["states"]:
            ...
            self.actionDiscoveryFeedMakeUpdatePost(
                "Food is being served in the cafeteria.", signals=["eatSignal"]
            )
        ...
        # self.pathfinder = Pathfinder()
            ...
            self.actionDiscoveryFeedMakeUpdatePost(
                "I'm going to the farm to collect mushrooms.", signals=[]
            )
            ...
            objsAtLocation = self.world.getObjectsAt(
                potNormalLocationX, potNormalLocationY
            )
            potContainer = None
            for obj in objsAtLocation:
                if obj.type == "table":
            ...
            self.addAutopilotActionToQueue(
                AutopilotAction_PickupObj(self.pot, priority=5)
            )
            ...
            self.addAutopilotActionToQueue(
                AutopilotAction_GotoXY(
                    x=fieldX + fieldWidth, y=fieldY + fieldHeight, priority=5
                )
            )
            ...
            self.addAutopilotActionToQueue(
                AutopilotAction_PostDiscoveryFeedUpdate(
                    contentStr="I'm going back to the cafeteria from the farm.",
                    signals=[],
                    priority=5,
                )
            )
            ...
            if potContainer is not None:
                self.addAutopilotActionToQueue(
                    AutopilotAction_PlaceObjInContainer(
                        self.pot, potContainer, priority=5
                    )
                )
            else:
                ## TODO: Make it drop the pot
                self.addAutopilotActionToQueue(
                    AutopilotAction_DropObjAtLocation(
                        dropX=21, dropY=21, objectNamesOrTypes=["pot"], priority=5
                    )
                )

            # Then, travel back to your starting location
            self.addAutopilotActionToQueue(
                AutopilotAction_GotoXY(x=20, y=21, priority=5)
            )

            # Then, make sure you're facing south
            self.addAutopilotActionToQueue(
                AutopilotAction_RotateToFaceDirection("south", priority=5)
            )
            ...
            self.actionDiscoveryFeedMakeUpdatePost(
                "I'm going to serve food in the cafeteria.", signals=[]
            )
            ...
            for i in range(0, min(5, len(edibleContents))):
                self.addAutopilotActionToQueue(
                    AutopilotAction_PlaceObjInContainer(
                        edibleContents[i], self.tables[i], priority=5
                    )
                )

            # Then, put the pot back down on the original table
            self.addAutopilotActionToQueue(
                AutopilotAction_PlaceObjInContainer(self.pot, potContainer, priority=5)
            )
...
class NPCColonistAuto2(NPC):
    ...
    def __init__(self, world, name, preferredX=15, preferredY=15):
        ...
        self.addAutopilotActionToQueue(
            AutopilotAction_Wander(preferredX=preferredX, preferredY=preferredY)
        )
    ...
    def tick(self):
        ...
        if hasMushroomInInventory:
            ...
            if not hasEatAction:
                ...
                self.addAutopilotActionToQueue(
                    AutopilotAction_PostDiscoveryFeedUpdate(
                        contentStr="I'm going to eat a mushroom.",
                        signals=[],
                        priority=3,
                    )
                )
                self.addAutopilotActionToQueue(
                    AutopilotAction_EatObjectInInventory(
                        objectNamesOrTypesToEat=["mushroom"], priority=3
                    )
                )
        ...
        if "poisoned" in self.attributes["states"]:
            ...
            self.addAutopilotActionToQueue(
                AutopilotAction_GotoXY(x=23, y=7, priority=100)
            )
        ...
        # self.pathfinder = Pathfinder()
        ...
        elif ("eatSignal" in self.attributes["states"]) or (
            ("eatSignal_" + self.name) in self.attributes["states"]
        ):
            ...
            self.actionDiscoveryFeedMakeUpdatePost(
                "Heading to the cafeteria for some food.", signals=[]
            )
            ...
            self.addAutopilotActionToQueue(
                AutopilotAction_GotoXY(x=23, y=25, priority=5)
            )
            ...
            # If false, try again
            actionTrue = AutopilotAction_EatObjectInInventory(
                objectNamesOrTypesToEat=["mushroom"], priority=5
            )
            actionFalse = AutopilotAction_PickupObjectsInArea(
                fieldX,
                fieldY,
                fieldWidth,
                fieldHeight,
            ...
            self.addAutopilotActionToQueue(
                AutopilotAction_CheckCondition(
                    conditionCallback=conditionCallback,
                    addActionIfTrue=actionTrue,
                    addActionIfFalse=actionFalse,
                    priority=5,
                )
            )
...
class NPCFarmer1(NPC):
    ...
    def tick(self):
        ...
        # self.pathfinder = Pathfinder()
            ...
            numSeedsToPlant = 5
            self.actionDiscoveryFeedMakeUpdatePost(
                "I'm going to try to plant some seeds. First I need a shovel, then some seeds.",
                signals=[],
            )
            ...
            for i in range(numSeedsToPlant):
                self.addAutopilotActionToQueue(
                    AutopilotAction_LocateBlankTileInArea(
                        farmX,
                        farmY,
                        farmWidth,
                        farmHeight,
                ...
                self.addAutopilotActionToQueue(
                    AutoPilotAction_BuryInFrontOfAgent(
                        objectNamesOrTypesToDig=["soil"],
                        objectNamesOrTypesToBury=["seed"],
                        priority=5,
                    )
                )
            ...
            # Then, travel back to your starting location
            self.addAutopilotActionToQueue(
                AutopilotAction_GotoXY(x=11, y=12, finalDirection="south", priority=5)
            )
...
class SoilController(NPCDevice):
    ...
    def __init__(self, world):
        ...
        Agent.__init__(
            ...
            "soil controller",
            "soil controller",
            defaultSpriteName="instruments_soil_controller",
        )

        self.spriteCharacterPrefix = ""  # Disable the character prefix for this object (just use the default sprite)

        # Default attributes
        self.attributes["isUsable"] = (
            True  # Can this device be used with another object? (e.g. specifically through the 'use' action)
        )
        self.attributes["isMovable"] = False  # Can it be moved?
    ...
    def setFieldNum(self, fieldNum, fieldTiles: list = []):
        ...
        dialogMaker.mkDialogSoilNutrientController(
            self, self.attributes["fieldNum"]
        )  # Sets the dialog tree
        tileUUIDs = [x.uuid for x in fieldTiles]
        self.attributes["fieldTileUUIDs"] = (
            tileUUIDs  # The tile UUIDs that this controller is responsible for
        )
    ...
    def tick(self):
        ...
        if "soilNutrientController_OK" in self.attributes["states"]:
            ...
            dialogMaker = DialogMaker()
            dialogMaker.mkDialogSoilNutrientControllerCompleted(
                self,
                fieldNum=self.attributes["fieldNum"],
                nutrientSettings=currentNutrientLevels,
            )
...
class CrystalReactor(NPCDevice):
    ...
    def __init__(self, world):
        ...
        Agent.__init__(
            ...
            "crystal reactor",
            "crystal reactor",
            defaultSpriteName="instruments_crystal_reactor_off",
        )
        ...
        self.attributes["containerPrefix"] = (
            "in"  # Container prefix (e.g. "in" or "on")
        )

        # Device (is activable)
        self.attributes["isActivatable"] = (
            False  # Is this a device? (more specifically, can it be activated/deactivated?)
        )
        ...
        self.attributes["resonanceFreq"] = (
            5000  # The resonance frequency of the crystal reactor
        )

        # Submission limit for reactor activation (controlled by environment variable)
        import os

        self.attributes["submissionCount"] = (
            0  # Number of times reactor has been activated
        )
        self.attributes["maxSubmissions"] = int(
            os.environ.get("REACTOR_MAX_SUBMISSIONS", "3")
        )  # Maximum number of submissions allowed

        # Display string (similar to PropertyManipulator's statusMessage)
        self.attributes["statusMessage"] = (
            "Crystal Reactor ready.\n"
            "Place a crystal inside and set the resonance frequency to match.\n"
            f"Usage: 0/{self.attributes['maxSubmissions']} submissions."
        )
        # Lock flag for PropertyManipulator: once the reactor dialog is opened, it stays locked.
        self.attributes["hasBeenUsed"] = False
        self.attributes["reactorOpened"] = False
        self.attributes["crystalPropertiesText"] = "(no crystal detected)"
    ...
    def setReactorNum(self, reactorNum: int):
        ...
        dialogMaker = DialogMaker()
        dialogMaker.mkDialogCrystalReactor(
            self, self.attributes["reactorNum"]
        )  # Sets the dialog tree
        # Also change the name of the reactor
        self.name = "crystal reactor " + str(self.attributes["reactorNum"])

    def _refreshCrystalPropertiesText(self):
        ...
        if containsCrystal is None:
            self.attributes["crystalPropertiesText"] = "(no crystal detected)"
            return

        # Only show physical, non-frequency crystal properties in reactor dialog.
        ...
        lines = []
        for key in physical_property_keys:
            if key not in containsCrystal.attributes:
                continue
            val = containsCrystal.attributes.get(key)
            if isinstance(val, (int, float)):
                lines.append(f"- {key}: {val}")

        self.attributes["crystalPropertiesText"] = (
            "\n".join(lines)
            if lines
            else "(no non-frequency physical properties available)"
        )

    def enteringDialogWith(self, agentTalkingTo):
        # Opening the reactor locks PropertyManipulator for the rest of the run.
        self.attributes["reactorOpened"] = True
        self.attributes["hasBeenUsed"] = True
        self._refreshCrystalPropertiesText()
    ...
    def checkResonanceFreq(self):
        ...
        if containsCrystal is not None:
            ...
            wiggleRoom = 5.0  # Allow 5 Hz of wiggle room in getting the value correct
            frequencyMatches = (
                abs(
                    self.attributes["resonanceFreq"]
                    - containsCrystal.attributes["resonanceFreq"]
                )
                < wiggleRoom
            )

            # Check submission limit before activating
            # Note: submissionCount is incremented in DialogTree when frequency is set
            submissionCount = self.attributes.get("submissionCount", 0)
            maxSubmissions = self.attributes.get("maxSubmissions", 3)

            if frequencyMatches:
                # Check if we've reached the submission limit
                if submissionCount >= maxSubmissions:
                    # Limit reached - don't activate
                    if self.attributes["isActivated"]:
                        self.needsSpriteNameUpdate = True
                    self.attributes["isActivated"] = False
                    self.name = f"crystal reactor (submission limit reached: {submissionCount}/{maxSubmissions})"
                    print(
                        f"Reactor submission limit reached: {submissionCount}/{maxSubmissions}"
                    )
                else:
                    # Crystal present, correct frequency, and under submission limit
                    wasActivated = self.attributes.get("isActivated", False)
                    if not wasActivated:
                        # Note: submissionCount is already incremented in DialogTree when frequency is set
                        # We just need to activate here
                        print(
                            f"Reactor activated. Submission count: {submissionCount}/{maxSubmissions}"
                        )
                        self.needsSpriteNameUpdate = True
            ...
            if freqReactorInt == freqCrystalInt:
                ...
                self.attributes["resonanceFreq"] = containsCrystal.attributes[
                    "resonanceFreq"
                ]
            else:
...
#
#   Instrument: Frequency Estimator (intervention machine)
#   The agent can TALK to this device and provide physical properties as JSON to receive a frequency.
#
class FrequencyEstimator(NPCDevice):
    # Constructor
    def __init__(self, world):
        Agent.__init__(
            self,
            ...
            "frequency estimator",
            "frequency estimator",
            defaultSpriteName="instruments_frequency_estimator",
        )
        ...
        # Parameters for mapping properties -> frequency. One of (linear) slope+offset, or (quadratic) a,b,c.
        self.attributes["keyDimension"] = 0
        self.attributes["slope"] = None
        self.attributes["offset"] = None
        self.attributes["a"] = None
        self.attributes["b"] = None
        self.attributes["c"] = None

        # Last result string for dialog display
        self.attributes["lastEstimatedFrequencyStr"] = "(no result yet)"

    def _get_key_value_from_props(self, props: dict):
        key_dim = self.attributes.get("keyDimension", 0)
        mode = self.attributes.get(
            "_key_mapping_mode", "normal"
        )  # 'normal', 'easy', 'challenge'

        try:
            if mode == "normal":
                # Normal mode mapping (mkCrystalProperties)
                if key_dim == 0:
                    return float(props.get("density"))
                if key_dim == 1:
                    # temperature in JSON may be provided as 'temperature' or 'temperatureC'
                    val = props.get("temperatureC", props.get("temperature"))
                    return float(val) if val is not None else None
                if key_dim == 2:
                    return float(props.get("quantumSize"))
                if key_dim == 3:
                    # radiation key in observation is 'radiationusvh'
                    val = props.get("radiationusvh", props.get("radiation"))
                    return float(val) if val is not None else None
                if key_dim == 4:
                    # spectrum channel 4; accept either array 'spectrum' or explicit 'spectrum_channel4'
                    if (
                        isinstance(props.get("spectrum"), list)
                        and len(props["spectrum"]) >= 5
                    ):
                        return float(props["spectrum"][4])
                    if "spectrum_channel4" in props:
                        return float(props["spectrum_channel4"])
                    return None

            elif mode == "easy":
                # Easy mode mapping (mkCrystalPropertiesEasy)
                if key_dim == 0:
                    return float(props.get("quantumSize"))
                if key_dim == 1:
                    return float(props.get("density"))
                if key_dim == 2:
                    ...
                    if (
                        isinstance(props.get("spectrum"), list)
                        and len(props["spectrum"]) >= 1
                    ):
                        return float(props["spectrum"][0])
                    if "spectrum_channel0" in props:
                        return float(props["spectrum_channel0"])
                    return None
                if key_dim == 3:
                    # temperature in JSON may be provided as 'temperature' or 'temperatureC'
                    val = props.get("temperatureC", props.get("temperature"))
                ...
                # Challenge mode mapping (mkCrystalPropertiesQuadratic)
                if key_dim == 0:
                    # temperature in JSON may be provided as 'temperature' or 'temperatureC'
                    val = props.get("temperatureC", props.get("temperature"))
                    return float(val) if val is not None else None
    ...
    def _estimate_frequency(self, key_value: float):
        a = self.attributes.get("a")
        b = self.attributes.get("b")
        c = self.attributes.get("c")
        slope = self.attributes.get("slope")
        offset = self.attributes.get("offset")
        try:
            if a is not None and b is not None and c is not None:
                return float(a) * (key_value**2) + float(b) * key_value + float(c)
            if slope is not None and offset is not None:
                return float(slope) * key_value + float(offset)
        except Exception:
            return None
        return None

    # DialogTree hook: handle JSON payloads sent via TALK
    def dialogHandleJSONInput(self, jsonIn: dict) -> bool:
        key_value = self._get_key_value_from_props(jsonIn)
        if key_value is None:
            self.attributes["lastEstimatedFrequencyStr"] = (
                "Missing or invalid properties for keyDimension="
                + str(self.attributes.get("keyDimension", 0))
            )
            return True
        est = self._estimate_frequency(key_value)
        if est is None:
            self.attributes["lastEstimatedFrequencyStr"] = (
                "Unable to compute frequency with current parameters."
            )
            return True
        # Clamp to reactor domain for consistency
        est = round(max(0.0, min(10000.0, float(est))), 2)
        self.attributes["lastEstimatedFrequencyStr"] = (
            f"Estimated frequency: {est} Hz (from key value {round(key_value, 4)})"
        )
        return True


#
#   Property Manipulator (Causal Discovery Machine)
#   A machine where agents can place a crystal, modify its properties (T or M_0),
#   and observe how frequency and moisture change based on causal relationships
#
class PropertyManipulator(NPCDevice):
    """
    Property Manipulator - A machine for exploring causal relationships

    Causal relationships implemented (T->F, T->M):
    - F = F_0 + k_f(T - T_ref)  [Temperature affects Frequency]
    - M = M_0 + k_d(T - T_ref)  [Temperature and Base Moisture affect Moisture]

    T (temperature) and M_0 (base moisture) are controllable.
    F (frequency) and M (moisture) are derived.
    """
    ...
    def __init__(self, world):
        Agent.__init__(
            ...
            "property manipulator",
            defaultSpriteName="instruments_centrifuge",
        )

        self.spriteCharacterPrefix = ""

        # Container attributes - can hold a crystal
        self.attributes["isContainer"] = True
        self.attributes["isOpenable"] = False
        self.attributes["isOpenContainer"] = True
        self.attributes["containerPrefix"] = "in"
        self.attributes["contentsVisible2D"] = True
        ...
        # Causal relationship parameters (from create_causal_graph_T_F_M)
        self.attributes["T_ref"] = 20.0  # Reference temperature
        self.attributes["F_0"] = 1000.0  # Base frequency
        self.attributes["k_f"] = 50.0  # Temperature-to-frequency coefficient
        self.attributes["k_d"] = (
            0.5  # Temperature-to-moisture coefficient (dehydration rate)
        )

        # Usage tracking for property manipulation
        self.attributes["usageCount"] = 0
        self.attributes["maxUses"] = int(
            os.environ.get("PROPERTY_MANIPULATOR_MAX_USES", "999999")
        )  # Default: unlimited

        # Display string
        self.attributes["statusMessage"] = (
            "No crystal inserted. Please place a crystal inside."
        )

    def enteringDialogWith(self, agentTalkingTo):
        reactor = self.attributes.get("reactor", None)
        if reactor is not None and reactor.attributes.get("hasBeenUsed", False):
            self.attributes["statusMessage"] = (
                "REACTOR ALREADY OPENED!\n"
                "You can no longer use the Property Manipulator after entering the reactor.\n"
                "Please continue reactor frequency setting and activation."
            )
            if hasattr(self, "dialogTree") and self.dialogTree is not None:
                if self.dialogTree.rootNodeName != "limitReachedNode":
                    self.dialogTree.setRoot("limitReachedNode")
                self.dialogTree.setCurrentNode("limitReachedNode")

    def getCrystal(self):
        """Get the crystal currently in the machine, if any"""
        for obj in self.contents:
            if obj.type == "quantum crystal":
                return obj
        return None

    def _get_hidden_variable_spec(self):
        """Return hidden-variable metadata if this manipulator's causal graph defines it."""
        causal_graph = self.attributes.get("causalGraph")
        if causal_graph is None:
            return None

        hidden_variable = getattr(causal_graph, "hidden_variable", None)
        if not isinstance(hidden_variable, dict):
            return None
        if not hidden_variable.get("enabled", False):
            return None
        affected_nodes = hidden_variable.get("affected_nodes", [])
        if not isinstance(affected_nodes, list) or len(affected_nodes) == 0:
            return None
        return hidden_variable

    def _get_current_hidden_value(self):
        """Read hidden variable's current value (defaults to configured initial value)."""
        hidden_spec = self._get_hidden_variable_spec()
        if hidden_spec is None:
            return 0.0

        if "hiddenVariableCurrentValue" not in self.attributes:
            self.attributes["hiddenVariableCurrentValue"] = float(
                hidden_spec.get("initial_value", 0.0)
            )
        return float(self.attributes.get("hiddenVariableCurrentValue", 0.0))

    def _get_hidden_phase_label(self):
        """Return a coarse phase label for debug logging."""
        reactor = self.attributes.get("reactor")
        if reactor is not None and reactor.attributes.get("hasBeenUsed", False):
            return "phase2_reactor"

        usage_count = self.attributes.get("usageCount", 0)
        if usage_count > 0:
            return "phase1_property_manipulator"
        return "phase1_pre_intervention"

    def _record_hidden_debug(
        self,
        property_name,
        requested_value,
        observed_intervened_value,
    ):
        """Persist the latest hidden-variable application for downstream tracking."""
        hidden_spec = self._get_hidden_variable_spec()
        if hidden_spec is None:
            self.attributes.pop("lastHiddenDebug", None)
            return None

        causal_graph = self.attributes.get("causalGraph")
        intervened_node = property_name
        if causal_graph is not None and hasattr(causal_graph, "node_property_mapping"):
            for node_name, prop_name in causal_graph.node_property_mapping.items():
                if prop_name == property_name:
                    intervened_node = node_name
                    break

        hidden_value = self._get_current_hidden_value()
        hidden_delta = round(
            float(hidden_spec.get("edge_weight", 0.0)) * hidden_value,
            2,
        )
        snapshot = {
            "enabled": True,
            "current_hidden_value": hidden_value,
            "hidden_delta": hidden_delta,
            "affected_nodes": list(hidden_spec.get("affected_nodes", [])),
            "intervened_node": intervened_node,
            "requested_value": requested_value,
            "observed_intervened_value": observed_intervened_value,
            "phase": self._get_hidden_phase_label(),
        }
        self.attributes["lastHiddenDebug"] = snapshot
        return snapshot

    def getHiddenDebugSnapshot(self, consume=False):
        """Return the latest hidden-variable debug payload for logging."""
        hidden_spec = self._get_hidden_variable_spec()
        if hidden_spec is None:
            return {
                "enabled": False,
        ...
        stored = self.attributes.get("lastHiddenDebug")
    ...
    def _advance_hidden_variable_after_intervention(self, rng):
        """
        Update hidden variable right after an intervention.
        Hidden stays at its initial value until the first intervention occurs.
        Every intervention afterwards samples from value_space, including the first one.
        """
        hidden_spec = self._get_hidden_variable_spec()
        if hidden_spec is None:
            return 0.0
        ...
        usage_count = self.attributes.get("usageCount", 0)
        if usage_count == 0:
            new_hidden_value = float(hidden_spec.get("initial_value", 0.0))
        else:
            value_space = hidden_spec.get("value_space", [-1, 0, 1])
            if not isinstance(value_space, list) or len(value_space) == 0:
                value_space = [-1, 0, 1]
            new_hidden_value = float(rng.choice(value_space))

        self.attributes["hiddenVariableCurrentValue"] = new_hidden_value
        return new_hidden_value

    def _get_causal_topological_order(self, causal_graph):
        """Return node order with parents before children."""
        order = []
        permanent = set()
        temporary = set()

        def visit(node_name):
            if node_name in permanent:
                return
            if node_name in temporary:
                raise ValueError(
                    f"Causal graph contains a cycle at node '{node_name}'"
                )

            temporary.add(node_name)
            for child_name in causal_graph.nodes[node_name].children:
                visit(child_name)
            temporary.remove(node_name)
            permanent.add(node_name)
            order.append(node_name)

        for node_name in causal_graph.nodes:
            visit(node_name)

        order.reverse()
        return order

    def _apply_hidden_effect_with_propagation(
        self,
        causal_graph,
        base_computed_values,
        current_values,
        hidden_spec,
        rng,
        precision=2,
        ...
        """
        Recompute the causal graph in topological order while injecting hidden effects
        at affected nodes, so downstream nodes see propagated changes.
        """
        hidden_value = self._get_current_hidden_value()
        hidden_weight = float(hidden_spec.get("edge_weight", 0.5))
        hidden_delta = hidden_weight * hidden_value
        if hidden_delta == 0:
            return base_computed_values

        affected_nodes = {
            node_name
            for node_name in hidden_spec.get("affected_nodes", [])
            if node_name in causal_graph.nodes
        }
        if len(affected_nodes) == 0:
            return base_computed_values

        last_base_values = getattr(causal_graph, "_last_base_values", {})
        propagated_values = {}
        topo_order = self._get_causal_topological_order(causal_graph)

        for node_name in topo_order:
            node = causal_graph.nodes[node_name]
            base_value = None

            if node.is_controllable:
                if node_name in current_values:
                    base_value = current_values[node_name]
                elif node_name in last_base_values:
                    base_value = last_base_values[node_name]
                elif node.computation_fn is None and node_name in base_computed_values:
                    base_value = base_computed_values[node_name]
            ...
            if node.computation_fn is None:
                if base_value is None:
                    value = base_computed_values.get(node_name, 0.0)
                else:
                    value = round(base_value, precision)
            else:
                parent_values = {
                    parent_name: propagated_values[parent_name]
                ...
                value = round(
                    node.computation_fn(parent_values, causal_graph.computation_params),
                    precision,
                )

            if node_name in affected_nodes:
                value = round(value + hidden_delta, precision)

            propagated_values[node_name] = value

        return propagated_values

    def _oracle_chains_enabled(self):
        return os.environ.get("PROPERTY_MANIPULATOR_ORACLE_CHAINS", "0") == "1"

    def _get_current_causal_base_values(self, crystal, causal_graph):
        current_values = {}
        for node_name, node in causal_graph.nodes.items():
            ...
            if base_attr in crystal.attributes:
                current_values[node_name] = crystal.attributes[base_attr]
            elif prop_name in crystal.attributes:
                current_values[node_name] = crystal.attributes[prop_name]
                crystal.attributes[base_attr] = crystal.attributes[prop_name]
        return current_values

    def _compute_causal_values(
        self,
        crystal,
        causal_graph,
        apply_hidden_effect=True,
    ):
        current_values = self._get_current_causal_base_values(crystal, causal_graph)
        computed_values = causal_graph.compute_values(
            crystal.rng, precision=2, override_base_values=current_values
        )

        if apply_hidden_effect:
            hidden_spec = self._get_hidden_variable_spec()
            if hidden_spec is not None:
                computed_values = self._apply_hidden_effect_with_propagation(
                    causal_graph=causal_graph,
                    base_computed_values=computed_values,
    ...
    def _append_oracle_chain_status(
        self,
        property_name,
        before_values,
        after_values,
    ):
        if not self._oracle_chains_enabled():
            self.attributes.pop("lastOracleInterventionChains", None)
            return
        ...
        for node_name, prop_name in causal_graph.node_property_mapping.items():
            if prop_name == property_name:
                ...
                break
        if target_node is None and property_name in causal_graph.nodes:
            target_node = property_name
        ...
        for node_name, node in causal_graph.nodes.items():
            for child_name in node.children:
                edge_pairs.append((node_name, child_name))

        chain_paths = build_maximal_changed_paths(
        ...
        if not chain_paths:
            # Keep an explicit status in oracle mode so runs are auditable even when
            # the intervened node has no changed descendants in the DAG.
            self.attributes["lastOracleInterventionChains"] = []
            self.attributes["statusMessage"] += (
                "\n  Oracle impact chains:\n"
                f"    {property_name} (no downstream changed descendants)"
            )
            return

        label_map = {
            node_name: causal_graph.node_property_mapping.get(node_name, node_name)
            for node_name in causal_graph.nodes
        }
        chain_strings = format_oracle_chain_strings(chain_paths, label_map)
        self.attributes["lastOracleInterventionChains"] = chain_strings
        self.attributes["statusMessage"] += "\n  Oracle impact chains:"
        for chain in chain_strings:
            self.attributes["statusMessage"] += f"\n    {chain}"

    def updateCrystalProperties(self, apply_hidden_effect=True):
        """Update the crystal's properties based on causal graph or legacy formulas"""
        crystal = self.getCrystal()
        if crystal is None:
            self.attributes["statusMessage"] = (
                "No crystal inserted. Please place a crystal inside."
            )
        ...
        # Check if reactor has been used (if reactor is linked, it locks PropertyManipulator)
        reactor = self.attributes.get("reactor", None)
        if reactor is not None:
            reactor_used = reactor.attributes.get("hasBeenUsed", False)
            if reactor_used:
                self.attributes["statusMessage"] = (
                    f"REACTOR ALREADY OPENED!\n"
                    f"The Crystal Reactor has been entered, which locks the Property Manipulator.\n"
                    f"No more property adjustments are allowed.\n"
                    f"Please proceed to activate the reactor with the correct frequency."
                )
                # Switch to limit reached node if dialog exists
                if hasattr(self, "dialogTree") and self.dialogTree is not None:
                    if "limitReachedNode" in self.dialogTree.nodes:
                        if self.dialogTree.rootNodeName != "limitReachedNode":
                            self.dialogTree.setRoot("limitReachedNode")
                        self.dialogTree.setCurrentNode("limitReachedNode")
                return

        # Check if we have a causal graph (new dynamic system)
        causal_graph = self.attributes.get("causalGraph")

        if causal_graph is not None:
            computed_values = self._compute_causal_values(
                crystal=crystal,
                causal_graph=causal_graph,
                apply_hidden_effect=apply_hidden_effect,
            )

            # Update crystal attributes with computed values
            status_lines = ["Crystal Analysis:"]
            for node_name, value in computed_values.items():
                prop_name = causal_graph.node_property_mapping.get(node_name, node_name)
                crystal.attributes[prop_name] = value

                # Add to status message, hiding values for unobservable nodes
                display_name = causal_graph.node_display_names.get(node_name, prop_name)
                observable = causal_graph.node_observable.get(node_name, True)
                shown_value = value if observable else "[unobservable]"
                status_lines.append(f"  {display_name}: {shown_value}")

            status_lines.append("  ───────────────────")

            # Add usage information
            usage_count = self.attributes.get("usageCount", 0)
            max_uses = self.attributes.get("maxUses", 999999)
            remaining = max_uses - usage_count
            if max_uses < 999999:  # Only show if there's a limit
                status_lines.append(
                    f"  Uses: {usage_count}/{max_uses} (Remaining: {remaining})"
                )

            self.attributes["statusMessage"] = "\n".join(status_lines)

        else:
            # Legacy system: hardcoded T->F, T->M relationships
            # Get current values
            T = crystal.attributes.get("temperatureC", 20.0)
            M_0 = crystal.attributes.get("moistureBase", 15.0)

            # Get causal parameters
            T_ref = self.attributes.get("T_ref", 20.0)
            F_0 = self.attributes.get("F_0", 1000.0)
            k_f = self.attributes.get("k_f", 50.0)
            k_d = self.attributes.get("k_d", 0.5)

            # Compute derived properties based on causal relationships
            # F = F_0 + k_f(T - T_ref)
            F = F_0 + k_f * (T - T_ref)
            F = round(F, 2)
            ...
            # Update crystal attributes
            crystal.attributes["resonanceFreq"] = F
            crystal.attributes["moisture"] = M
            ...
            self.attributes["statusMessage"] = status_msg

    def adjustProperty(self, property_name, amount, is_absolute=False):
        """
        Generic method to adjust any controllable property

        In causal-graph mode, controllable properties adjust their BASE value
        (including hybrid nodes). Observed values are then recomputed.

        Args:
            property_name: The property attribute name (e.g., 'temperatureC', 'moisture', 'density')
            amount: The amount to adjust by (positive or negative) if is_absolute=False,
                    or the absolute value to set if is_absolute=True
            is_absolute: If True, set property to the absolute value 'amount'.
                        If False (default), add 'amount' to current value.

        Returns:
            bool: True if successful, False if no crystal or limit reached
        """
        crystal = self.getCrystal()
        if crystal is None:
            return False
        ...
        if reactor is not None:
            ...
            if reactor_used:
                print(
                    f"[adjustProperty] Reactor has been used. PropertyManipulator is now locked."
                )
                self.attributes["statusMessage"] = (
                    f"REACTOR ALREADY OPENED!\n"
                    f"The Crystal Reactor has been entered, which locks the Property Manipulator.\n"
                ...
                # Switch to limit reached node if it exists
                if hasattr(self, "dialogTree") and self.dialogTree is not None:
                    if "limitReachedNode" in self.dialogTree.nodes:
                        if self.dialogTree.rootNodeName != "limitReachedNode":
                            self.dialogTree.setRoot("limitReachedNode")
        ...
        if usage_count >= max_uses:
            print(
                f"[adjustProperty] Usage limit reached ({usage_count}/{max_uses}). Cannot adjust property."
            )
            self.attributes["statusMessage"] = (
                f"USAGE LIMIT REACHED!\n"
                f"Property Manipulator has been used {usage_count}/{max_uses} times.\n"
        ...
        self.attributes["usageCount"] = usage_count + 1
        print(f"[adjustProperty] Usage: {self.attributes['usageCount']}/{max_uses}")

        # Check if we just reached the usage limit
        if self.attributes["usageCount"] >= max_uses:
            # Immediately switch dialog tree to limit reached state
            if hasattr(self, "dialogTree") and self.dialogTree is not None:
                if self.dialogTree.rootNodeName != "limitReachedNode":
                    self.dialogTree.setRoot("limitReachedNode")
                # Force current node to limit reached node
                self.dialogTree.setCurrentNode("limitReachedNode")
                print(
                    "[adjustProperty] Switched to limitReachedNode after reaching usage limit"
                )

        # Check if this property is a hybrid node (controllable with computation)
        causal_graph = self.attributes.get("causalGraph")
        is_hybrid = False
        is_causal_controllable = False
        oracle_before_values = None

        if causal_graph is not None:
            # Find the node for this property
            for node_name, node in causal_graph.nodes.items():
                prop_name = causal_graph.node_property_mapping.get(node_name, node_name)
                if prop_name == property_name:
                    # Hybrid if it's controllable AND has a computation
                    is_causal_controllable = node.is_controllable
                    is_hybrid = node.is_controllable and node.computation_fn is not None
                    break
            if self._oracle_chains_enabled():
                oracle_before_values = self._compute_causal_values(
                    crystal=crystal,
                    causal_graph=causal_graph,
                    apply_hidden_effect=False,
                )

        # Determine which attribute to adjust
        if is_causal_controllable:
            # For controllable nodes in causal mode, always adjust base value.
            attr_to_adjust = f"{property_name}_base"
            current_value = crystal.attributes.get(
                attr_to_adjust, crystal.attributes.get(property_name, 0.0)
            )

            # Initialize base if it doesn't exist
            if attr_to_adjust not in crystal.attributes:
                crystal.attributes[attr_to_adjust] = current_value
        else:
            # Legacy/non-causal behavior: adjust the property value directly
            attr_to_adjust = property_name
            current_value = crystal.attributes.get(property_name, 0.0)

        # Calculate new value based on mode
        if is_absolute:
            if is_hybrid:
                # For hybrid nodes, we need to set the base such that the final computed value equals the target
                # First, compute what the current final value is
                # Use causal-only values here (without hidden perturbation) to solve base correctly.
                self.updateCrystalProperties(apply_hidden_effect=False)
                current_final_value = crystal.attributes.get(property_name, 0.0)
                
                # The contribution from parents (non-base factors)
                # contribution = current_final_value - current_base_value
                contribution = current_final_value - current_value
                
                # IMPORTANT: Apply bounds to the TARGET FINAL VALUE first, not the base
                # This ensures the final computed value respects the property's valid range
                target_final_clamped = amount  # Will be clamped below
                
                # # Apply property-specific bounds to target final value
                # if property_name == "temperatureC":
                #     target_final_clamped = max(0.0, min(100.0, target_final_clamped))
                # elif property_name == "moisture":
                #     target_final_clamped = max(0.0, min(100.0, target_final_clamped))
                # elif property_name == "density":
                #     target_final_clamped = max(0.0, min(100.0, target_final_clamped))
                # elif property_name == "quantumSize":
                #     target_final_clamped = max(0.1, min(100.0, target_final_clamped))
                # elif property_name == "radiation":
                #     target_final_clamped = max(0.0, min(100.0, target_final_clamped))
                # elif property_name == "pressure":
                #     target_final_clamped = max(0.0, min(100.0, target_final_clamped))
                # elif property_name == "ph":
                #     target_final_clamped = max(0.0, min(14.0, target_final_clamped))
                # elif property_name == "conductivity":
                #     target_final_clamped = max(0.0, min(100.0, target_final_clamped))
                # elif property_name == "resonanceFreq":
                #     target_final_clamped = max(0.0, min(10000.0, target_final_clamped))
                # else:
                #     # Generic bounds for unknown properties
                #     target_final_clamped = max(0.0, min(1000.0, target_final_clamped))
                
                # To achieve clamped target final value:
                # target_final_value = new_base_value + contribution
                # Therefore: new_base_value = target_final_value - contribution
                new_value = target_final_clamped - contribution
                
                print(
                    f"[adjustProperty] Hybrid property {property_name}: "
                    f"target final value = {amount} (clamped to {target_final_clamped}), "
                    f"current final value = {current_final_value}, "
                    f"current base = {current_value}, "
                    f"contribution from parents = {contribution}, "
                    f"setting base to = {new_value}"
                )
            else:
                # For pure controllable nodes, direct value setting (like reactor frequency)
                new_value = amount
        else:
            # Incremental adjustment (legacy behavior)
            new_value = current_value + amount

        # Apply reasonable bounds based on property type
        # For hybrid nodes with absolute mode, bounds were already applied to target final value above
        # For pure controllable nodes or incremental mode, apply bounds to the new_value (base or property value)
        # if not (is_absolute and is_hybrid):  # Skip if already bounded for hybrid absolute mode
        #     if property_name == "temperatureC":
        ...
        #         new_value = max(0.0, min(100.0, new_value))
        #     elif property_name == "ph":
        #         new_value = max(0.0, min(14.0, new_value))
        #     elif property_name == "conductivity":
        #         new_value = max(0.0, min(100.0, new_value))
        #     elif property_name == "resonanceFreq":
        #         # This shouldn't be adjustable, but just in case
        #         new_value = max(0.0, min(10000.0, new_value))
        #     else:
        #         # Generic bounds for unknown properties
        #         new_value = max(0.0, min(1000.0, new_value))

        new_value = round(new_value, 2)
        crystal.attributes[attr_to_adjust] = new_value

        # Log the change
        if is_hybrid:
            if is_absolute:
                print(
                    f"[adjustProperty] {property_name} (hybrid): base set to {new_value} (was {current_value})"
                )
            else:
                print(
                    f"[adjustProperty] {property_name} (hybrid): adjusted base from {current_value} to {new_value}"
                )
        else:
            if is_absolute:
                print(
                    f"[adjustProperty] {property_name}: set to {new_value} (was {current_value})"
                )
            else:
                print(
                    f"[adjustProperty] {property_name}: adjusted from {current_value} to {new_value}"
                )

        # Hidden variable drifts after each intervention.
        self._advance_hidden_variable_after_intervention(crystal.rng)

        # Update all derived properties based on causal relationships + hidden effect.
        self.updateCrystalProperties(apply_hidden_effect=True)
        if oracle_before_values is not None and causal_graph is not None:
            oracle_after_values = self._compute_causal_values(
                crystal=crystal,
                causal_graph=causal_graph,
                apply_hidden_effect=False,
            )
            self._append_oracle_chain_status(
                property_name=property_name,
                before_values=oracle_before_values,
                after_values=oracle_after_values,
            )
        observed_value = crystal.attributes.get(property_name)
        requested_value = amount if is_absolute else new_value
        try:
            requested_value = round(float(requested_value), 2)
        except Exception:
            pass
        try:
            ...
            pass
        self._record_hidden_debug(
            property_name=property_name,
            requested_value=requested_value,
            observed_intervened_value=observed_value,
        )
        return True

    # Legacy methods for backward compatibility
    def increaseTemperature(self, amount):
        """Increase the crystal's temperature by a specific amount"""
        return self.adjustProperty("temperatureC", amount)

    def decreaseTemperature(self, amount):
        """Decrease the crystal's temperature by a specific amount"""
        return self.adjustProperty("temperatureC", -amount)

    def increaseMoisture(self, amount):
        """Increase the crystal's moisture"""
        return self.adjustProperty("moisture", amount)

    def decreaseMoisture(self, amount):
        """Decrease the crystal's moisture"""
        return self.adjustProperty("moisture", -amount)

    def increaseDensity(self, amount):
        """Increase the crystal's density"""
        return self.adjustProperty("density", amount)

    def decreaseDensity(self, amount):
        """Decrease the crystal's density"""
        return self.adjustProperty("density", -amount)

    def increaseQuantumSize(self, amount):
        """Increase the crystal's quantum size"""
        return self.adjustProperty("quantumSize", amount)

    def decreaseQuantumSize(self, amount):
        """Decrease the crystal's quantum size"""
        return self.adjustProperty("quantumSize", -amount)

    def increaseRadiation(self, amount):
        """Increase the crystal's radiation"""
        return self.adjustProperty("radiation", amount)

    def decreaseRadiation(self, amount):
        """Decrease the crystal's radiation"""
        return self.adjustProperty("radiation", -amount)

    def increasePressure(self, amount):
        """Increase the crystal's pressure"""
        return self.adjustProperty("pressure", amount)

    def decreasePressure(self, amount):
        """Decrease the crystal's pressure"""
        return self.adjustProperty("pressure", -amount)

    def increasePH(self, amount):
        """Increase the crystal's pH"""
        return self.adjustProperty("ph", amount)

    def decreasePH(self, amount):
        """Decrease the crystal's pH"""
        return self.adjustProperty("ph", -amount)

    def increaseConductivity(self, amount):
        """Increase the crystal's conductivity"""
        return self.adjustProperty("conductivity", amount)

    def decreaseConductivity(self, amount):
        """Decrease the crystal's conductivity"""
        return self.adjustProperty("conductivity", -amount)

    def tick(self):
        """Update the display on each tick"""
        self.updateCrystalProperties()

        # Check if usage limit has been reached and update dialog root
        usage_count = self.attributes.get("usageCount", 0)
        max_uses = self.attributes.get("maxUses", 999999)

        if usage_count >= max_uses:
            # Switch to limit reached node
            if hasattr(self, "dialogTree") and self.dialogTree is not None:
                if self.dialogTree.rootNodeName != "limitReachedNode":
                    self.dialogTree.setRoot("limitReachedNode")
                    # If currently in dialog and not on limit/end node, redirect to limit node
                    if self.dialogTree.currentNodeName not in [
                        "limitReachedNode",
                        "endNode",
                    ]:
                        self.dialogTree.setCurrentNode("limitReachedNode")
        else:
            # Make sure we're using the normal root node
            if hasattr(self, "dialogTree") and self.dialogTree is not None:
                if self.dialogTree.rootNodeName != "rootNode":
                    self.dialogTree.setRoot("rootNode")
