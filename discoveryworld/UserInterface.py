...
from discoveryworld.Agent import Agent, CrystalReactor
...
class UserInterface:
    ...
    def __init__(self, window, spriteLibrary, showScoreToUser=False):
        ...
        self.argObjectsList = (
            []
        )  # The (most recently updated) list of all objects that can be selected as arguments
        ...
        # Text input state for dialog numeric input
        self.textInputActive = False
        self.textInputBuffer = ""
        self.textInputPrompt = ""
        self.selectedDialogOptionForInput = None
    ...
    #
    def startTextInput(self, prompt, selectedDialogOption):
        """Start text input mode for numeric dialog input"""
        self.textInputActive = True
        self.textInputBuffer = ""
        self.textInputPrompt = prompt
        self.selectedDialogOptionForInput = selectedDialogOption

    def cancelTextInput(self):
        """Cancel text input mode"""
        # If we're in frequency input node, return to root node when canceling
        if self.currentAgent and self.currentAgent.isInDialog():
            agentInDialogWith = self.currentAgent.getAgentInDialogWith()
            if (
                agentInDialogWith
                and agentInDialogWith.dialogTree.currentNodeName == "frequencyInputNode"
            ):
                # Return to root node
                agentInDialogWith.dialogTree.setCurrentNode("rootNode")
                # Update dialog display
                npcResponse, nextDialogOptions = (
                    agentInDialogWith.dialogTree.getCurrentDialog()
                )
    ...
    def handleTextInputEvent(self, event):
        """Handle pygame keyboard events for text input"""
        ...
        if event.type == pygame.KEYDOWN:
            if event.key == pygame.K_RETURN or event.key == pygame.K_KP_ENTER:
                ...
                self.textInputBuffer = self.textInputBuffer[:-1]
            else:
                # Add character (only allow numbers, decimal point, and minus sign)
                char = event.unicode
                if char in "0123456789.-":
                    self.textInputBuffer += char
        return False

    def getTextInputValue(self):
        """Get the current text input as a float"""
        try:
            return float(self.textInputBuffer) if self.textInputBuffer else None
        except ValueError:
            return None

    def isDialogOptionRequiringNumericInput(self, dialogOption):
        """Check if a dialog option requires numeric input

        Note: Only reactor frequency setting uses this. PropertyManipulator
        uses increment/decrement options, not direct numeric input.
        """
        numericOptions = [
            "Set frequency to a specific value",
        ]
        return dialogOption in numericOptions

    def submitTextInput(self):
        """Submit the text input and execute the dialog action"""
        if not self.textInputActive or not self.currentAgent.isInDialog():
            return (
                False,
                ActionSuccess(success=False, message="Not in text input mode"),
            )

        # Get the value
        value = self.getTextInputValue()
        if value is None:
            return (
                False,
                ActionSuccess(success=False, message="Invalid numeric value"),
            )

        # Get the agent we're talking to
        agentInDialogWith = self.currentAgent.getAgentInDialogWith()
        selectedDialogOption = self.selectedDialogOptionForInput

        # Special handling for frequency input node (direct numeric input, no option selection)
        if agentInDialogWith.dialogTree.currentNodeName == "frequencyInputNode":
            # Directly send the numeric value to the dialog tree
            # The dialog tree will handle setting the frequency and returning to root
            actionResult = self.currentAgent.actionDialog(
                agentToTalkTo=agentInDialogWith, dialogStrToSay=str(value)
            )
        # Special handling for property input nodes (PropertyManipulator)
        elif agentInDialogWith.dialogTree.currentNodeName.endswith("InputNode"):
            # Directly send the numeric value to the dialog tree
            # The dialog tree will handle setting the property value
            actionResult = self.currentAgent.actionDialog(
                agentToTalkTo=agentInDialogWith, dialogStrToSay=str(value)
            )
        else:
            # Normal flow: Step 1: Say the option
            if selectedDialogOption:
                actionResult = self.currentAgent.actionDialog(
                    agentToTalkTo=agentInDialogWith, dialogStrToSay=selectedDialogOption
                )
            # Step 2: Say the numeric value
            actionResult = self.currentAgent.actionDialog(
                agentToTalkTo=agentInDialogWith, dialogStrToSay=str(value)
            )
    ...
    def popMessageFromQueue(self):
        ...
        if len(self.messageQueueText) > 0:
            ...
            print(
                "\tAfter removing one: Messages in queue: "
                + str(len(self.messageQueueText))
            )
        if len(self.messageQueueText) == 0:
            self.inDiscoveryFeedModal = False
    ...
    def selectArgumentBox(self, idx, whichBox):
        ...
        if whichBox == 1:
            ...
            self.curSelectedArgument1Obj = self.argObjectsList[
                self.curSelectedArgument1Idx
            ]
        elif whichBox == 2:
            self.curSelectedArgument2Idx = idx
            self.curSelectedArgument2Obj = self.argObjectsList[
                self.curSelectedArgument2Idx
            ]
        else:
    ...
    def changeArgumentBox(self, delta, whichBox):
        ...
        if self.curSelectedArgument1Idx >= 0 and self.curSelectedArgument1Idx < len(
            self.argObjectsList
        ):
            self.curSelectedArgument1Obj = self.argObjectsList[
                self.curSelectedArgument1Idx
        ...
        if self.curSelectedArgument2Idx >= 0 and self.curSelectedArgument2Idx < len(
            self.argObjectsList
        ):
            self.curSelectedArgument2Obj = self.argObjectsList[
                self.curSelectedArgument2Idx
    ...
    def render(self):
        ...
        if self.currentAgent != None:
            ...
            objsEnv += self.currentAgent.getObjectsAgentFacing(
                respectContainerStatus=True
            )
            # Also include objects on adjacent N/E/S/W tiles as accessible candidates
            objsEnv += self._getAdjacentNESWObjects(respectContainerStatus=True)

        objsEnv = self._filterEnvObjects(
            objsInv, objsEnv
        )  # Filter any env objects that are also in the inventory
        allObjs = objsInv + objsEnv
        allObjs = self._filterDuplicateObjects(
            allObjs
        )  # Filter out any duplicates (e.g. the 'agent facing objects' function is sometimes setup to also return objects in the agent's current square, including its own inventory)
        ...
        self.changeArgumentBox(
            delta=0, whichBox=1
        )  # Bound checking/Make sure the references to the selected objects are up to date
        self.renderObjectSelectionBox(
            objsInv,
            objsEnv,
            self.curSelectedArgument1Idx,
            offsetX=32,
            offsetY=-(32 * 9) + 20,
            labelPrefixStr="Arg 1: ",
        )
        ...
        if self.dialogToDisplay != None:
            ...
            # If text input is active, add the input prompt and current input
            if self.textInputActive:
                dialogBoxStr += "\n" + self.textInputPrompt + "\n"
                dialogBoxStr += "> " + self.textInputBuffer + "_\n"
        ...
        # Turn counter
        if self.currentAgent != None:
            textSurface = self.fontBold.render(
                "     Turn " + str(self.currentAgent.world.getStepCounter()),
                True,
                (128, 128, 128),
            )
        ...
        if self.currentAgent != None:
            ...
            if newPosts > 0:
                ...
                pygame.draw.circle(
                    self.window, (255, 0, 0), (helpX + 110, helpY - 125), 5
                )
                if pygame.time.get_ticks() % 1000 < 500:
                    pygame.draw.circle(
                        self.window, (0, 0, 0), (helpX + 110, helpY - 125), 5, 1
                    )

                textSurface = self.fontBold.render(
                    "       " + str(newPosts) + " New", True, (200, 200, 200)
                )
                self.window.blit(textSurface, (helpX - 10, helpY - 132))
                textSurface = self.fontBold.render(
                    "    DiscoveryFeed", True, (200, 200, 200)
                )
                self.window.blit(textSurface, (helpX - 10, helpY - 112))
                if numPosts > 1:
                    textSurface = self.fontBold.render(
                        "       Post(s)", True, (200, 200, 200)
                    )
                else:
                    textSurface = self.fontBold.render(
                        "        Post", True, (200, 200, 200)
                    )
        ...
        if self.currentAgent != None:
            ...
            if task != None and task.isCompleted():
                ...
                if (
                    (self.inModal == False)
                    and (self.dialogToDisplay == None)
                    and (self.extendedPlayEnabled == False)
                ):
                    ...
                    pygame.draw.rect(
                        self.window,
                        (100, 100, 100),
                        (taskCompletedX - 175, taskCompletedY - 5, 500, 50),
                    )
                    # Then, render the text
                    textSurface = self.fontBold.render(
                        "Task Completed", True, (200, 200, 200)
                    )
                    self.window.blit(textSurface, (taskCompletedX + 25, taskCompletedY))
                    # Check if the task was completed successfully
                    if task.isCompletedSuccessfully() == True:
                        textSurface = self.fontBold.render(
                            "Press ESC to quit, or F5 to keep playing this scenario.",
                            True,
                            (200, 200, 200),
                        )
                        self.window.blit(
                            textSurface, (taskCompletedX - 170, taskCompletedY + 20)
                        )
                    else:
                        textSurface = self.fontBold.render(
                        ...
                        self.window.blit(
                            textSurface, (taskCompletedX + 15, taskCompletedY + 20)
                        )
    ...
    def renderJSON(self):
        ...
        validDirections, blockedDirections = (
            self.currentAgent.getValidDirectionsToMoveTo()
        )
        ...
        nearbyObjectsFull, nearbyObjects, nearbyObjectsByDirection = (
            self.currentAgent.getNearbyVisibleObjects(
                maxDistance=nearbyObjectsMaxDistance, includeUUID=True
            )
        )
        # Note: nearbyObjectsByDirection is smaller
        out["nearbyObjects"] = {
            "note": "The objects below are within "
            + str(nearbyObjectsMaxDistance)
            + " tiles of the agent, but may not neccesarily be usable if they're not in the agent inventory, or directly in front of the agent.  This list should help in navigating to objects you'd like to interact with or use.  Objects to interact with or use should be in the 'accessibleEnvironmentObjects' or 'inventoryObjects' lists.",
        ...
        out["discoveryFeed"] = self.currentAgent.world.discoveryFeed.getRecentPosts(
            curStep=self.currentAgent.world.getStepCounter(), lastNSteps=3
        )

        # Pop-up boxes/Dialog
        dialogBoxDict = {}
        # Use agent state as source of truth for dialog flag
        dialogBoxDict = {
            "is_in_dialog": (
                self.currentAgent.isInDialog()
                if self.currentAgent is not None
                else False
            )
        }
        if (self.currentAgent is not None) and (self.currentAgent.isInDialog()):
            # Ensure dialogToDisplay is populated when in dialog
            if self.dialogToDisplay == None:
                agentInDialogWith = self.currentAgent.getAgentInDialogWith()
                npcResponse, nextDialogOptions = (
                    agentInDialogWith.dialogTree.getCurrentDialog()
    ...
    def getRecentActionHistoryOfAgents(self, nearbyObjectList):
        ...
        out = {}
        out["description"] = (
            "This section lists the recent action history (i.e. within the last few steps) of any agents that are nearby. This can help you understand what other agents are doing, and what they might be planning to do."
        )
        out["list_of_agents"] = {}
        for agent in agentList:
            history = agent.actionHistory.exportToJSONAbleListHumanReadable(
                lastNSteps=3
            )
    ...
    def renderInventory(self):
        ...
        for idx in range(inventorySlots):
            ...
            if idx == self.curSelectedInventoryIdx:
                self.spriteLibrary.renderSprite(
                    self.window, "ui_inventory_selected", x, y - tileSize, scale
                )
            else:
                self.spriteLibrary.renderSprite(
                    self.window, "ui_inventory_background", x, y - tileSize, scale
                )
    ...
    def _renderObjectSprite(self, object, x, y, scale=1.0):
        ...
        for spriteDict in object.getSpriteNamesWithContents():
            self.spriteLibrary.renderSprite(
                ...
                x + spriteDict["xOffset"],
                y + spriteDict["yOffset"],
                scale,
                adjustY=True,
            )
    ...
    def renderObjectSelectionBox(
        self, objsInv, objsEnv, curSelectedObjIdx, offsetX, offsetY, labelPrefixStr=""
    ):
        ...
        for col in range(numCols):
            ...
            if objIdx == curSelectedObjIdx:
                self.spriteLibrary.renderSprite(
                    self.window, "ui_inventory_selected", x, y, scale
                )
            else:
                if objIdx >= len(objectList):
                    # Empty inventory spot
                    self.spriteLibrary.renderSprite(
                        self.window, "ui_inventory_empty", x, y, scale
                    )
                elif objIdx >= len(objsInv):
                    # Environmental inventory spot (slightly different background)
                    self.spriteLibrary.renderSprite(
                        self.window, "ui_inventory_background_env", x, y, scale
                    )
                else:
                    # Inventory spot
                    self.spriteLibrary.renderSprite(
                        self.window, "ui_inventory_background", x, y, scale
                    )
        ...
        for col in range(numCols):
            ...
            if objIdx < endIdx:
                ...
                self._renderObjectSprite(
                    obj, x + xAdj, y + tileSize - yAdj, scale=scale1
                )
        ...
        x = 0 * (32 * scale) + offsetX
        y = self.window.get_height() + offsetY + (tileSize * scale)
        label = self.fontBold.render(
            labelPrefixStr + selectedObjectName, 1, (255, 255, 255)
        )
    ...
    def renderObjectSelectionBoxJSON(self, objsInv, objsEnv):
        ...
        # Pack
        out = {"inventoryObjects": invOut, "accessibleEnvironmentObjects": envOut}
        # Return
        return out
    ...
    def renderTextBox(self, text):
        ...
        pygame.draw.rect(
            self.window,
            (128, 128, 128),
            (startX - 16, startY - 16, totalWidth + 32, totalHeight + 32),
            0,
        )
    ...
    # Collect objects on adjacent N/E/S/W tiles (excluding the current tile);
    # used to broaden what the UI and action parser consider accessible.
    def _getAdjacentNESWObjects(self, respectContainerStatus=True):
        objs = []
        if self.currentAgent is None or self.currentAgent.world is None:
            return objs
        x = self.currentAgent.attributes["gridX"]
        y = self.currentAgent.attributes["gridY"]
        positions = [(x, y - 1), (x + 1, y), (x, y + 1), (x - 1, y)]
        for nx, ny in positions:
            if self.currentAgent.world.isWithinBounds(nx, ny):
                objs_at = self.currentAgent.world.getObjectsAt(
                    nx,
                    ny,
    ...
    def actionDeactivateObject(self, objToDeactivate):
        success = self.currentAgent.actionActivateDeactivate(
            objToDeactivate, "deactiviate"
        )
        return success
    ...
    def actionTalk(self, agentToTalkTo):
        ...
        success = self.currentAgent.actionDialog(
            agentToTalkTo=agentToTalkTo, dialogStrToSay="Hello!"
        )
        return success
    ...
    def getDiscoveryFeedUpdates(self, startFromID=None):
        ...
        self.lastDiscoveryFeedPostCount = len(
            self.currentAgent.world.discoveryFeed.getPosts()
        )
    ...
    def createDiscoveryFeedUpdate(self, contentStr, signals: list = []):
        ...
        success = self.currentAgent.actionDiscoveryFeedMakeUpdatePost(
            contentStr, signals
        )
        return success
    ...
    def _parseDialogKeys(self, keys):
        ...
        if keys[pygame.K_0]:
            ...
            # Get the current dialog state to check if we're in an input node
            agentInDialogWith = self.currentAgent.getAgentInDialogWith()
            if agentInDialogWith:
                npcResponse, nextDialogOptions = (
                    agentInDialogWith.dialogTree.getCurrentDialog()
                )
                current_node_name = agentInDialogWith.dialogTree.currentNodeName
                # Special handling: in input nodes, ESC returns to previous menu
                if current_node_name == "frequencyInputNode" or current_node_name.endswith("InputNode"):
                    # Cancel text input if active
                    if self.textInputActive:
                        self.cancelTextInput()
                    # Select the back option (index 0)
                    # For frequencyInputNode: "Back to main menu"
                    # For property InputNodes: "Back to adjustment menu"
                    if len(nextDialogOptions) >= 1:
                        selectedDialogOption = nextDialogOptions[0]
                        actionResult = self.currentAgent.actionDialog(
                            agentToTalkTo=agentInDialogWith,
                            dialogStrToSay=selectedDialogOption,
                        )
                        # Update dialog display
                        npcResponse, nextDialogOptions = (
                            agentInDialogWith.dialogTree.getCurrentDialog()
                        ...
                        self.dialogToDisplay["dialogOptions"] = nextDialogOptions
                        return (True, actionResult)
            # In other nodes, ESC exits the dialog
            self.currentAgent.exitDialog()
        ...
        # Special handling for input nodes (frequency input and property input nodes)
        # In these nodes, ALL number keys (0-9) are used for numeric input, not for selecting options
        current_node_name = agentInDialogWith.dialogTree.currentNodeName
        if current_node_name == "frequencyInputNode" or current_node_name.endswith("InputNode"):
            # All number keys should be used for numeric input, not for selecting options
            # ESC key is handled above to return to main menu/adjustment menu
            # Start text input mode if not already active
            if not self.textInputActive:
                if current_node_name == "frequencyInputNode":
                    self.startTextInput("Enter frequency value (0-10000 Hz):", None)
                else:
                    # Extract property name for display
                    prop_name = current_node_name[:-9]  # Remove "InputNode"
                    self.startTextInput(f"Enter value for {prop_name}:", None)
            # Don't process the key here - let text input events handle it
            # All number keys (0-9) will be handled by handleTextInputEvent for numeric input
            return (
                False,
                ActionSuccess(
                    success=False,
                    message="Type numeric value or press ESC to return",
                ),
            )
        ...
        if key > len(nextDialogOptions):
            return (
                False,
                ActionSuccess(success=False, message="Unknown dialog option."),
            )

        # If the key is in range, then select that dialog option
        selectedDialogOption = nextDialogOptions[key - 1]

        # Say that response back to the NPC (this will transition to the next node)
        actionResult = self.currentAgent.actionDialog(
            agentToTalkTo=agentInDialogWith, dialogStrToSay=selectedDialogOption
        )
        ...
        # Check if we've transitioned to the frequency input node and start text input automatically
        # This happens AFTER the dialog transition, so we're now in the frequencyInputNode
        if agentInDialogWith.dialogTree.currentNodeName == "frequencyInputNode":
            if not self.textInputActive:
                self.startTextInput("Enter frequency value (0-10000 Hz):", None)
        
        # Check if we've transitioned to a property input node and start text input automatically
        # Pattern: <propertyName>InputNode (e.g., temperatureCInputNode, moistureInputNode)
        if agentInDialogWith.dialogTree.currentNodeName.endswith("InputNode") and agentInDialogWith.dialogTree.currentNodeName != "frequencyInputNode":
            if not self.textInputActive:
                # Extract property name for display
                prop_name = agentInDialogWith.dialogTree.currentNodeName[:-9]  # Remove "InputNode"
                self.startTextInput(f"Enter value for {prop_name}:", None)
    ...
    def _parseDialogJSON(self, jsonIn):
        ...
        # Special handling for frequency input node: agent can send frequency directly
        if agentInDialogWith.dialogTree.currentNodeName == "frequencyInputNode":
            # In frequency input node, agent can send frequency field directly
            if "value" in jsonIn:
                try:
                    freq_value = float(jsonIn["value"])
                    # Send the value directly to the dialog tree
                    actionResult = self.currentAgent.actionDialog(
                        agentToTalkTo=agentInDialogWith,
                        dialogStrToSay={"value": freq_value},
                    )
                    # Update dialog display
                    npcResponse, nextDialogOptions = (
                        agentInDialogWith.dialogTree.getCurrentDialog()
                    ...
                    return (
                        ...
                        ActionSuccess(
                            success=False,
                            message="Invalid frequency value. Must be a number.",
                        ),
                    )
            else:
                return (
                    ...
                    ActionSuccess(
                        success=False,
                        message="In frequency input mode, provide 'value' field with numeric value.",
                    ),
                )
        
        # Special handling for property input nodes: agent can send property value directly
        if agentInDialogWith.dialogTree.currentNodeName.endswith("InputNode") and agentInDialogWith.dialogTree.currentNodeName != "frequencyInputNode":
            # In property input node, agent can send value field directly
            if "value" in jsonIn:
                try:
                    prop_value = float(jsonIn["value"])
                    # Send the property value directly to the dialog tree
                    actionResult = self.currentAgent.actionDialog(
                        agentToTalkTo=agentInDialogWith,
                        dialogStrToSay={"value": prop_value},
                    )
                    # Update dialog display
                    npcResponse, nextDialogOptions = (
                        agentInDialogWith.dialogTree.getCurrentDialog()
                    ...
                    return (
                        ...
                        ActionSuccess(
                            success=False,
                            message="Invalid property value. Must be a number.",
                        ),
                    )
            else:
                return (
                    ...
                    ActionSuccess(
                        success=False,
                        message="In property input mode, provide 'value' field with numeric value.",
                    ),
                )

        # If no explicit dialog option is provided, allow JSON payloads for instrument-style dialogs
        if "chosen_dialog_option_int" not in jsonIn:
            # Try passing the JSON directly to the dialog tree (some devices accept JSON inputs)
            try:
                actionResult = self.currentAgent.actionDialog(
                    agentToTalkTo=agentInDialogWith, dialogStrToSay=jsonIn
                )
                # Refresh dialog display state
                npcResponse, nextDialogOptions = (
                    agentInDialogWith.dialogTree.getCurrentDialog()
                )
                self.dialogToDisplay = {}
                ...
                return (
                    ...
                    ActionSuccess(
                        success=False,
                        message="Missing `chosen_dialog_option_int` and JSON input not accepted for this dialog.",
                    ),
                )

        # If we're in frequency input node, we shouldn't have options, so chosen_dialog_option_int shouldn't be used
        if agentInDialogWith.dialogTree.currentNodeName == "frequencyInputNode":
            return (
                False,
                ActionSuccess(
                    success=False,
                    message="In frequency input mode, provide 'value' field instead of 'chosen_dialog_option_int'.",
                ),
            )
        
        # If we're in property input node, we shouldn't have options, so chosen_dialog_option_int shouldn't be used
        if agentInDialogWith.dialogTree.currentNodeName.endswith("InputNode") and agentInDialogWith.dialogTree.currentNodeName != "frequencyInputNode":
            return (
                False,
                ActionSuccess(
                    success=False,
                    message="In property input mode, provide 'value' field instead of 'chosen_dialog_option_int'.",
                ),
            )
        ...
        if isinstance(chosenDialogOption, int) == False:
            ...
            try:
                ...
                return (
                    ...
                    ActionSuccess(
                        success=False,
                        message="`chosen_dialog_option_int` is not an integer.",
                    ),
                )
        ...
        if chosenDialogOption < 1:
            return (
                False,
                ActionSuccess(
                    success=False, message="`chosen_dialog_option_int` is less than 1."
                ),
            )
        if chosenDialogOption > len(nextDialogOptions):
            return (
                ...
                ActionSuccess(
                    success=False,
                    message="`chosen_dialog_option_int` is greater than number of options available ("
                    + str(len(nextDialogOptions))
                    + ").",
                ),
            )
        ...
        # Update dialog display after transition
        npcResponse, nextDialogOptions = agentInDialogWith.dialogTree.getCurrentDialog()
        self.dialogToDisplay = {}
        self.dialogToDisplay["dialogText"] = npcResponse
        self.dialogToDisplay["dialogOptions"] = nextDialogOptions

        # Special handling: if we transitioned to frequencyInputNode and agent provided frequency
        if agentInDialogWith.dialogTree.currentNodeName == "frequencyInputNode":
            if "value" in jsonIn:
                # Agent provided value in the same JSON, apply it immediately
                try:
                    freq_value = float(jsonIn["value"])
                    # Send the value directly to the dialog tree
                    actionResult = self.currentAgent.actionDialog(
                    ...
                    # Update dialog display again (should now be back at rootNode)
                    npcResponse, nextDialogOptions = (
                        agentInDialogWith.dialogTree.getCurrentDialog()
                    )
                    self.dialogToDisplay = {}
            ...
            # If no frequency provided, agent needs to send it in next action
            # The dialog will remain in frequencyInputNode until frequency is provided
    ...
    def parseKeys(self, keys):
        ...
        if keys[pygame.K_SPACE]:
            checkArgSuccess = self._checkArgs(
                actionName="pick up object", arg1=True, arg2=False
            )
            ...
            return (
                True,
                self.actionPickupObject(objToPickUp=self.curSelectedArgument1Obj),
            )

        # Drop an inventory item in arg1 slot at the agents current location
        elif keys[pygame.K_d]:
            checkArgSuccess = self._checkArgs(
                actionName="drop item", arg1=True, arg2=False
            )
            if checkArgSuccess == False:
                return (False, False)
        ...
        # Put an item (arg1) in a specific container (arg2)
            ...
            checkArgSuccess = self._checkArgs(
                actionName="put item in container", arg1=True, arg2=True
            )
            if checkArgSuccess == False:
                return (False, False)
            ...
            return (
                True,
                self.actionPutObjectInContainer(
                    objToPut=self.curSelectedArgument1Obj,
                    container=self.curSelectedArgument2Obj,
                ),
            )
        ...
        # Close a container or passage (arg1)
            ...
            return (
                True,
                self.actionCloseObject(objToClose=self.curSelectedArgument1Obj),
            )

        # Activate an object (arg1)
        elif keys[pygame.K_a]:
            checkArgSuccess = self._checkArgs(
                actionName="activate", arg1=True, arg2=False
            )
            if checkArgSuccess == False:
                return (False, False)
            ...
            return (
                True,
                self.actionActivateObject(objToActivate=self.curSelectedArgument1Obj),
            )
        ...
        # Deactivate an object (arg1)
            ...
            checkArgSuccess = self._checkArgs(
                actionName="deactivate", arg1=True, arg2=False
            )
            if checkArgSuccess == False:
                return (False, False)
            ...
            return (
                True,
                self.actionDeactivateObject(
                    objToDeactivate=self.curSelectedArgument1Obj
                ),
            )
        ...
        # Dialog/talk with agent specified in arg1
            ...
            if self.currentAgent.isInDialog():
                npcResponse, nextDialogOptions = (
                    self.currentAgent.getAgentInDialogWith().dialogTree.getCurrentDialog()
                )
                self.dialogToDisplay = {}
                self.dialogToDisplay["dialogText"] = npcResponse
                self.dialogToDisplay["dialogOptions"] = nextDialogOptions
        ...
        # Use action (use arg1 with arg2)
            ...
            checkArgSuccess = self._checkArgs(
                actionName="use object", arg1=True, arg2=True
            )
            if checkArgSuccess == False:
                return (False, False)

            result = self.actionUse(
                objToUse=self.curSelectedArgument1Obj,
                objToUseWith=self.curSelectedArgument2Obj,
            )
        ...
        # Teleport to a random (named) location
            ...
            teleportLocationNames = (
                self.currentAgent.world.getTeleportLocations().keys()
            )
            if len(teleportLocationNames) == 0:
                return (
                    False,
                    ActionSuccess(
                        success=False, message="No teleport locations found."
                    ),
                )
        ...
        whichBox = 1
        message = (
            lambda: f"Changed argument box 1 to {self.curSelectedArgument1Obj.name if self.curSelectedArgument1Obj else None}"
        )
        if keys[pygame.K_LSHIFT] or keys[pygame.K_RSHIFT]:
            whichBox = 2
            message = (
                lambda: f"Changed argument box 2 to {self.curSelectedArgument2Obj.name if self.curSelectedArgument2Obj else None}"
            )
        ...
        if keys[pygame.K_LEFTBRACKET]:
            ...
            return (
                ...
                ActionSuccess(
                    success=True,
                    message="Changed argument box 1 to "
                    + str(self.curSelectedArgument1Obj.name),
                ),
            )
            ...
            return (
                ...
                ActionSuccess(
                    success=True,
                    message="Changed argument box 2 to "
                    + str(self.curSelectedArgument2Obj.name),
                ),
            )
        ...
        if keys[pygame.K_v]:
            ...
            numPosts = len(self.currentAgent.world.discoveryFeed.getPosts())
            self.discoveryFeedStartIdx = (
                numPosts - 6
            )  # By default, start at the end (show the last 7 posts)
            return (
                False,
                self.getDiscoveryFeedUpdates(startFromID=self.discoveryFeedStartIdx),
            )
    ...
    def discoveryFeedViewIncrement(self):
        ...
        numPosts = len(self.currentAgent.world.discoveryFeed.getPosts())
        self.discoveryFeedStartIdx += (
            7  # By default, start at the end (show the last 7 posts)
        )
    ...
    def _checkArgs(self, actionName: str, arg1: bool, arg2: bool):
        ...
        if arg1 and arg2:
            if (
                self.curSelectedArgument1Obj == None
                and self.curSelectedArgument2Obj == None
            ):
                return ActionSuccess(
                    success=False,
                    message="Action '"
                    + actionName
                    + "' requires two argument objects. Missing both arguments.",
                )
            if self.curSelectedArgument1Obj == None:
                return ActionSuccess(
                    ...
                    message="Action '"
                    + actionName
                    + "' requires two argument objects. Missing argument 1.",
                )
            if self.curSelectedArgument2Obj == None:
                return ActionSuccess(
                    ...
                    message="Action '"
                    + actionName
                    + "' requires two argument objects. Missing argument 2.",
                )
        ...
        # Only argument 1 is required
            ...
            if self.curSelectedArgument1Obj == None:
                return ActionSuccess(
                    success=False,
                    message="Action '" + actionName + "' requires argument 1.",
                )
        ...
        # Only argument 2 is required
            ...
            if self.curSelectedArgument2Obj == None:
                return ActionSuccess(
                    success=False,
                    message="Action '" + actionName + "' requires argument 2.",
                )
    ...
    def _convertJSONArgsToObjects(self, jsonIn):
        ...
        accessibleObjs += self.currentAgent.getObjectsAgentFacing(
            respectContainerStatus=True
        )
        # Also include objects on adjacent N/E/S/W tiles for argument resolution
        accessibleObjs += self._getAdjacentNESWObjects(respectContainerStatus=True)
        ...
        if ("arg1" in jsonIn) and (jsonIn["arg1"] != None):
            ...
            if arg1Obj == None:
                ...
                if type(jsonIn["arg1"]) != int:
                    typeErrorStr = (
                        "Note: expected type of arg1 is 'int', but found `"
                        + str(type(jsonIn["arg1"]))
                        + "`. "
                    )
                errors.append(
                    "arg1: Could not find object with UUID '"
                    + str(jsonIn["arg1"])
                    + "'. Are you sure it's accessible (i.e in your inventory, or directly in front of you?). "
                    + typeErrorStr
                )
        ...
        if ("arg2" in jsonIn) and (jsonIn["arg2"] != None):
            ...
            if arg2Obj == None:
                ...
                if type(jsonIn["arg2"]) != int:
                    typeErrorStr = (
                        "Note: expected type of arg2 is 'int', but found `"
                        + str(type(jsonIn["arg2"]))
                        + "`. "
                    )
                errors.append(
                    "arg2: Could not find object with UUID '"
                    + str(jsonIn["arg2"])
                    + "' Are you sure it's accessible (i.e in your inventory, or directly in front of you?). "
                    + typeErrorStr
                )
    ...
    def parseActionJSON(self, jsonIn):
        ...
        if "action" not in jsonIn:
            ...
            return (
                ...
                ActionSuccess(
                    False,
                    "Missing 'action' key in JSON. You are not currently in dialog mode -- for non-dialog actions, expected a dictionary containing this key, as well as 'arg1' and 'arg2' keys (as appropriate).",
                ),
            )
        ...
        # Move in a specific direction (north/east/south/west)
            ...
            if "arg1" not in jsonIn:
                ...
                return (
                    False,
                    jsonParseErrors,
                    ActionSuccess(False, "Missing 'arg1' key in JSON."),
                )
            if jsonIn["arg1"] not in ["north", "east", "south", "west"]:
                jsonParseErrors.append(
                    "Invalid direction '" + str(jsonIn["arg1"]) + "' specified in JSON."
                )
                return (
                    False,
                    jsonParseErrors,
            ...
            return (
                True,
                jsonParseErrors,
                self.currentAgent.actionMoveAgentNorthEastSouthWest(direction),
            )
        ...
        # Rotate to a specific direction (north/east/south/west)
            ...
            return (
                True,
                jsonParseErrors,
                self.currentAgent.actionRotateAgentFacingDirectionAbsolute(direction),
            )
        ...
        # Teleport action (teleport to a specific location)
            ...
            return (
                True,
                jsonParseErrors,
                self.currentAgent.actionTeleportAgentToLocation(location),
            )
        ...
        # Teleport action (teleport to a specific object)
            ...
            return (
                True,
                jsonParseErrors,
                self.currentAgent.actionTeleportAgentToObject(obj),
            )

        # # Directly set reactor frequency
        # elif (jsonIn["action"] == ActionType.SET_REACTOR_FREQUENCY.name):
        #     # arg1: reactor UUID; frequency: target frequency (int/float)
        #     # Validate arg1 exists among accessible objects
        #     if ('arg1' not in jsonIn):
        #         jsonParseErrors.append("Missing 'arg1' key in JSON.")
        #         return (False, jsonParseErrors, ActionSuccess(False, "Missing 'arg1' key in JSON."))
        #     if ('frequency' not in jsonIn):
        #         jsonParseErrors.append("Missing 'frequency' key in JSON.")
        #         return (False, jsonParseErrors, ActionSuccess(False, "Missing 'frequency' key in JSON."))

        #     # Resolve arg1 to object (already in _convertJSONArgsToObjects we set curSelectedArgument1Obj if possible)
        #     reactorObj = self.curSelectedArgument1Obj
        #     if reactorObj is None or reactorObj.uuid != jsonIn['arg1']:
        #         # try to resolve manually from accessible objects
        #         accessibleObjs = self.currentAgent.getInventory()
        ...
        #                 reactorObj = obj
        #                 break
        #     if reactorObj is None:
        #         return (False, jsonParseErrors, ActionSuccess(False, "Could not find reactor object with UUID '" + str(jsonIn['arg1']) + "'."))

        #     # Validate type
        #     from discoveryworld.Agent import CrystalReactor
        #     if not isinstance(reactorObj, CrystalReactor):
        #         return (False, jsonParseErrors, ActionSuccess(False, "Object specified by 'arg1' is not a crystal reactor."))

        #     # Parse frequency
        #     try:
        #         freq = float(jsonIn['frequency'])
        #     except Exception:
        #         return (False, jsonParseErrors, ActionSuccess(False, "'frequency' must be a number."))

        #     # Clamp to allowable range 0..10000
        #     freq = max(0, min(10000, freq))
        #     reactorObj.attributes['resonanceFreq'] = freq
        #     # Trigger consistency checks
        #     reactorObj.checkResonanceFreq()
        #     msg = f"Set reactor {reactorObj.attributes.get('reactorNum', '')} frequency to {reactorObj.attributes['resonanceFreq']} Hz."
        #     return (True, jsonParseErrors, ActionSuccess(True, msg))
        ...
        # Drop an inventory item in arg1 slot at the agents current location
            ...
            return (
                True,
                jsonParseErrors,
                self.actionDropObject(objToDrop=self.curSelectedArgument1Obj),
            )
        ...
        # Open a container or passage (arg1)
            ...
            return (
                True,
                jsonParseErrors,
                self.actionOpenObject(objToOpen=self.curSelectedArgument1Obj),
            )
        ...
        # Eat arg1
            ...
            return (
                True,
                jsonParseErrors,
                self.actionEat(objToEat=self.curSelectedArgument1Obj),
            )
        ...
        # Read arg1
            ...
            return (
                True,
                jsonParseErrors,
                self.actionRead(objToRead=self.curSelectedArgument1Obj),
            )
        ...
        # Use action (use arg1 with arg2)
            ...
            if result.success == False:
                ...
                result1 = self.actionUse(
                    objToUse=self.curSelectedArgument2Obj,
                    objToUseWith=self.curSelectedArgument1Obj,
                )
                if result1.success == True:
                    result = result1
                    # Append a message saying the arguments were switched
                    result.message += (
                        "\n Note, the original action failed (USE, arg1:"
                        + self.curSelectedArgument1Obj.name
                        + ", arg2:"
                        + self.curSelectedArgument2Obj.name
                        + "), but the arguments were switched and the action succeeded (USE, arg1:"
                        + self.curSelectedArgument2Obj.name
                        + ", arg2:"
                        + self.curSelectedArgument1Obj.name
                        + "). Try to enter the arguments in the correct order next time."
                    )
        ...
        # Reading articles
            ...
            return (
                False,
                jsonParseErrors,
                self.getDiscoveryFeedUpdates(startFromID=None),
            )
        elif jsonIn["action"] == ActionType.DISCOVERY_FEED_GET_ARTICLES.name:
            return (
                False,
                jsonParseErrors,
                self.getDiscoveryFeedArticles(startFromID=None),
            )
            ...
            if "arg1" in jsonIn:
                try:
                    ...
                    return (
                        ...
                        ActionSuccess(
                            success=False,
                            message="Invalid post ID specified in JSON, or post ID could not be converted to an integer.",
                        ),
                    )
            else:
                return (
                    ...
                    ActionSuccess(
                        success=False,
                        message="Missing 'arg1' key (containing the post ID, as an integer) in JSON.",
                    ),
                )
            ...
            return (
                False,
                jsonParseErrors,
                self.getSpecificDiscoveryFeedPost(postID=postID),
            )
        # Creating articles
            ...
            return (
                False,
                jsonParseErrors,
                self.createDiscoveryFeedUpdate(contentStr="This is a test update."),
            )
        elif jsonIn["action"] == ActionType.DISCOVERY_FEED_CREATE_ARTICLE.name:
            return (
                False,
                jsonParseErrors,
                self.createDiscoveryFeedArticle(
                    titleStr="Test Article", contentStr="This is a test article."
                ),
            )
        ...
        return (
            ...
            jsonParseErrors,
            ActionSuccess(
                success=False, message="Unknown action '" + str(jsonIn["action"]) + "'."
            ),
        )
