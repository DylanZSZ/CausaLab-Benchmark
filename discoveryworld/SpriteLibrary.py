    ...
    def loadSpritesheet(self, spritesheetData):
        ...
        for spriteName in sprites:
            ...
            spriteRect = pygame.Rect(
                (spriteX * tileSize[0]) + offset[0],
                (spriteY * tileSize[1]) + offset[1],
                spriteWidthInTiles * tileSize[0],
                spriteHeightInTiles * tileSize[1],
            )
            ...
            if spriteImage.get_width() < 32:
                ...
                newSpriteImage = pygame.Surface(
                    (32, spriteImage.get_height()), pygame.SRCALPHA, 32
                )
                # Blit the original sprite onto the new surface
                newSpriteImage.blit(
                    spriteImage, (0, 0), special_flags=pygame.BLEND_RGBA_ADD
                )
            ...
            if spriteImage.get_height() < 32:
                # Create a new surface
                newSpriteImage = pygame.Surface(
                    (spriteImage.get_width(), 32), pygame.SRCALPHA, 32
                )
                # Blit the original sprite onto the new surface
                newSpriteImage.blit(
                    spriteImage, (0, 0), special_flags=pygame.BLEND_RGBA_ADD
            ...
            if spriteColor is not None:
                ...
                colorImage.blit(
                    spriteImage, (0, 0), special_flags=pygame.BLEND_RGBA_MULT
                )
            ...
            if sheetName + "_" + spriteName in self.sprites:
                self.warnings.append(
                    "WARNING: Duplicate sprite name: "
                    + sheetName
                    + "_"
                    + spriteName
                    + ". Sprite will be overwritten."
                )
