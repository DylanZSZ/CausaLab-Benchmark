...
class DiscoveryWorldAPI:
    ...
    def loadScenario(self, scenarioName:str, difficultyStr:str, randomSeed:int=0, numUserAgents = 1, causal_config=None):
        ...
        success, errorStr = scenarioMaker.setupScenario(internalScenarioName, self.numUserAgents, causal_config=causal_config)
            ...
            for filename in os.listdir(self.FRAME_DIR):
                if filename.endswith(".png"):
                    try:
                        ...
                        # File may have been deleted by another parallel process, ignore
                        pass
    ...
    def _get_property_manipulator(self):
        if self.world is None or getattr(self.world, "taskScorer", None) is None:
            return None

        for task in getattr(self.world.taskScorer, "tasks", []):
            scoring_info = getattr(task, "scoringInfo", None)
            if not isinstance(scoring_info, dict):
                continue
            prop_manip = scoring_info.get("propertyManipulator")
            if prop_manip is not None:
                return prop_manip
        return None

    def getHiddenDebugInfo(self):
        """Return a compact hidden-variable debug payload for rollout logging."""
        prop_manip = self._get_property_manipulator()
        if prop_manip is None or not hasattr(prop_manip, "getHiddenDebugSnapshot"):
            return {
                "enabled": False,
        ...
        try:
            return prop_manip.getHiddenDebugSnapshot(consume=True)
